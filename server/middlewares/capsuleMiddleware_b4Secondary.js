...
async function __getKeywordIdsByNames (selectedWords) {
	...
	var keywordIds = [];
	for(var i = 0; i < results.length; i++) {
		keywordIds.push(results[i].gt_id);
	}
	return keywordIds;
}
...
var streamPost__getAllWordsFromPythonApi = async function (req, res, next) {
	...
	var primary = Array.isArray(keywordObj.Primary) ? keywordObj.Primary : [];
	var secondary = Array.isArray(keywordObj.Secondary) ? keywordObj.Secondary : [];
	...
	if(req.body.SurpriseSelectedWords.length === 0 && primary.length === 2) {
		...
		req.body.SecondaryKeywords = secondary || [];
	}
