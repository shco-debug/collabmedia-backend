...
/**fetchKeywordsFromText_PrimarySecondary - Using Gemini api */
async function fetchKeywordsFromText_PrimarySecondary (text) {
    ...
    if(inputText) {
        ...
        while (true) {
            try {
                ...
                console.log(`Attempt ${attempt + 1} failed. Error from Gemini Api = `, err);
    ...
    csvkeywords = openaiResponse.data.candidates[0].content.parts[0].text.trim();
	...
	response.data = {
        Primary: primaryKeywords, //csvkeywordsArr.slice(0,2).map((obj)=>obj.trim()),
        Secondary: secondaryKeywords, //csvkeywordsArr.slice(2).map((obj)=>obj.trim())
