...
var create = async function (req, res) {
  try {
    ...
    data = {
      CapsuleId: req.body.capsule_id,
      CreaterId: req.session && req.session.user ? req.session.user._id : "68a733773931522f1b7f4632",
      OwnerId: req.session && req.session.user ? req.session.user._id : "68a733773931522f1b7f4632",
      IsPublished: req.body.is_published ? req.body.is_published : false,
      Title: req.body.title,
      Order: req.body.order ? req.body.order : 0,
      Origin: req.body.origin ? req.body.origin : "created",
    };

    // Handle LaunchSettings if provided
    if (req.body.launch_settings && typeof req.body.launch_settings === "object") {
      data.LaunchSettings = {
        MakingFor: req.body.launch_settings.making_for || "ME",
        ShareMode: req.body.launch_settings.share_mode || "friend-group",
        NamingConvention: req.body.launch_settings.naming_convention || "realnames",
        Invitees: req.body.launch_settings.invitees || [],
        OthersData: req.body.launch_settings.others_data || []
      };
    }
    ...
    await pushChapterId(data.CapsuleId, result._id);
    res.json(response);
  } catch (err) {
    console.log(err);
    var response = {
                          ...
                          function (err, result) {
                            ...
                            if (data.PageType == "content") {
                              ...
                              for (
                                var loop = 0;
                                loop <
                                data.ViewportDesktopSections.Widgets.length;
                                loop++
                              ) {
                                var widObj =
                                  data.ViewportDesktopSections.Widgets[loop];
                                widObj.Type = widObj.Type ? widObj.Type : "";
                                if (widObj.Type == "questAnswer") {
                                  // If Widget is a QA Widget then ...
                                  widObj.QAWidObj = widObj.QAWidObj
                              ...
                              for (
                                var loop = 0;
                                loop <
                                data.ViewportTabletSections.Widgets.length;
                                loop++
                              ) {
                                var widObj =
                                  data.ViewportTabletSections.Widgets[loop];
                                if (widObj.Type == "questAnswer") {
                                  // If Widget is a QA Widget then ...
                                  widObj.QAWidObj = widObj.QAWidObj
                                    ? widObj.QAWidObj
                              ...
                              for (
                                var loop = 0;
                                loop <
                                data.ViewportMobileSections.Widgets.length;
                                loop++
                              ) {
                                var widObj =
                                  data.ViewportMobileSections.Widgets[loop];
                                if (widObj.Type == "questAnswer") {
                                  // If Widget is a QA Widget then ...
                                  widObj.QAWidObj = widObj.QAWidObj
                                    ? widObj.QAWidObj
                              ...
                              async.series(
                                {
                                  createNewInstance__HiddenBoard: function (
                                    callback
                                  ) {
                                    if (
                                      finalObj.UNIQUE__allHiddenBoardId_Arr
                                        .length
                                    ) {
                                      var conditions = {
                                        _id: {
                                  ...
                                  if (!err) {
                                    ...
                                    for (
                                      ...
                                      for (
                                        var loop2 = 0;
                                        loop2 <
                                        finalObj.margedArrOfAllQAPageIds.length;
                                        loop2++
                                      ) {
                                        var recordArr2 =
                                          finalObj.margedArrOfAllQAPageIds[
        ...
        setTimeout(function () {
          if (fs.existsSync(srcPath)) {
            ...
            var dstPathCrop_SMALL =
              __dirname + mediaCenterPath + "small" + "/" + imgUrl;
            var dstPathCrop_MEDIUM =
              __dirname + mediaCenterPath + "medium" + "/" + imgUrl;
...
var page__sendInvitations = function (PageData, invitees, req) {
    ...
    User.find({ Email: shareWithEmail }, { Name: true }, function (err, name) {
      ...
      subject =
        subject != ""
          ...
          : "Scrpt - " +
            req.session.user.Name +
            " has invited you in his space page to join!";
  ...
  var ChapterViewURL =
    process.HOST_URL + "/space/" + PageData.ChapterId + "/" + PageData._id;
...
var invite = function (req, res) {
  if (req.headers.page_id) {
    //check whether it is Journal/Space Module case or not?
    ...
    invitee.relation = req.body.invitee.relation
      ? req.body.invitee.relation
      : "";
      ...
      Chapter.find(
        {
          ...
          "LaunchSettings.Invitees": {
            $elemMatch: {
              UserEmail: { $regex: new RegExp(invitee.email, "i") },
            },
          },
        },
        function (errr, dataa) {
                        ...
                        function (err1, data2) {
                          if (err1) {
                            ...
                            Chapter.update(
                              ...
                              {
                                $push: {
                                  "LaunchSettings.Invitees": newInvitee,
                                },
                              },
                              { multi: false },
                              function (err, data3) {
...
var invite__pageLevel = function (req, res) {
  ...
  Page.find(
    { _id: chapter_id, LaunchSettings: { $exists: false } },
    {},
    function (errr, dataa) {
      if (!errr) {
        dataa = dataa ? dataa : [];
...
var invite__pageLevel_Final = function (req, res) {
  console.log(
    "in invite__pageLevel_Final function========================================",
    req.body
  );
  var chapter_id = req.headers.page_id;
  var invitee = {};
                  ...
                  Chapter.findOne(
                    ...
                    function (err, result) {
                      if (!err) {
                        ...
                        AppController.__capsule_touchUpdateFlag(
    ...
    function (errr, dataa) {
      ...
      if (errr) {
        ...
        if (dataa.length == 0) {
          ...
          User.find(
            { Email: member.UserEmail, IsDeleted: 0 },
            { Name: 1, NickName: 1, ProfilePic: 1 },
            function (err, userRecord) {
              if (!err) {
                userRecord = userRecord ? userRecord : [];
                member.UserName = userRecord[0].Name
                  ? userRecord[0].Name
                  : member.UserName;
                member.UserNickName = userRecord[0].NickName
                  ? userRecord[0].NickName
                  : member.UserNickName;
                member.UserPic = userRecord[0].ProfilePic
                  ? userRecord[0].ProfilePic
...
var addOwner = function (req, res) {
  ...
  if (new RegExp(email, "i").test(req.session.user.Email)) {
    ...
    Chapter.find(
      {
        _id: chapter_id,
        "LaunchSettings.OthersData": {
          $elemMatch: { UserEmail: { $regex: new RegExp(email, "i") } },
        },
      },
      function (errr, dataa) {
        if (errr) {
              ...
              function (err, data) {
                if (err) {
                  ...
                  if (data != undefined && data != null) {
                    ...
                    Chapter.update(
                      { _id: chapter_id },
                      { $push: { "LaunchSettings.OthersData": newOwner } },
                      { multi: false },
                      function (err, data3) {
                        if (err) {
                          var response = {
...
var all__getLaunchedChapters = function (req, res) {
  ...
  var conditions = {
    ...
    $or: [
      ...
      { CreaterId: req.session.user._id, Origin: { $ne: "createdForMe" } },
      { OwnerId: req.session.user._id, OwnerEmail: req.session.user.Email },
    ],
    Status: 1,
    IsLaunched: 1,
    IsDeleted: 0,
    ...
    .exec(function (err, results) {
      if (!err) {
        ...
        for (var loop = 0; loop < results.length; loop++) {
          var srcPath =
            __dirname +
            "/../../public/views/intro-pages/" +
            String(results[loop]._id) +
            ".html";
      ...
      if (!err) {
        ...
        form.parse(req, function (err, fields, files) {
          ...
          if (files.myFile.name) {
            ...
            var media_type = "";
            if (
              files.myFile.type == "application/pdf" ||
              files.myFile.type == "application/msword" ||
              files.myFile.type ==
                "application/vnd.openxmlformats-officedocument.wordprocessingml.document" ||
              files.myFile.type == "application/vnd.ms-excel" ||
              files.myFile.type ==
                "application/vnd.oasis.opendocument.spreadsheet" ||
              files.myFile.type ==
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" ||
              files.myFile.type == "application/vnd.ms-powerpoint" ||
              files.myFile.type ==
                "application/vnd.openxmlformats-officedocument.presentationml.presentation"
            ) {
              media_type = "Document";
            } else if (
              files.myFile.type == "video/mp4" ||
              files.myFile.type == "video/ogg"
            ) {
              media_type = "Video";
            } else if (
              files.myFile.type == "audio/mpeg" ||
              files.myFile.type == "audio/ogg"
            ) {
              media_type = "Audio";
            } else {
              media_type = "Image";
                  ...
                  im.identify(srcPath, function (err, features) {
                    if (err) {
                      ...
                      if (features.width == 50 && features.height == 50) {
                        var dstPath =
                          __dirname + mediaCenterPath + "resized/" + imgUrl;
...
var pushChapterId = async function (capsuleId, chapterId) {
  try {
    // Skip if capsuleId is invalid (0 or falsy)
    if (!capsuleId || capsuleId === 0) {
      console.log("Skipping capsule update - invalid capsuleId:", capsuleId);
      return;
    }
...
var updateChapterForPageId = async function (req, res) {
  try {
    // Accept chapter_id from headers or body (as per original function)
    const chapterId = req.headers.chapter_id || req.body.chapter_id;
    
    if (!chapterId) {
      return res.json({
        status: 400,
        message: "Chapter ID is required in headers or body.",
      });
    }
    
    // Find the chapter first
    const chapter = await Chapter.findOne({ _id: chapterId }).exec();
    ...
    const pages = await Page.find(conditions).exec();
    
    console.log("Searching:", pages);
    
    if (pages.length) {
      // Update chapter with all pages at once
      const pageIds = pages.map(page => page._id);
      
      await Chapter.updateOne(
        { _id: chapterId },
        { $push: { pages: { $each: pageIds } } }
      );
      
      console.log("Pages saved in chapter successfully.");
    ...
    console.log("Error in updateChapterForPageId:", err);
    res.json({
      status: 500,
      message: "Something went wrong.",
      error: err.message,
...
var deleteAudioToChapter = function (req, res) {
  ...
  if (playListId != null) {
    Chapter.update(
      { _id: conditions._id },
      { $pull: { ChapterPlaylist: { _id: playListId } } },
      function (err, data) {
        if (err) {
          console.log(err);
        } else {
          Chapter.findOne({ _id: conditions._id }).exec(function (
            err,
            results
          ) {
            if (err) {
...
var chapterIntroDetails = function (req, res) {
  if (req.body.chapter_id) {
    Chapter.findOne({ _id: req.body.chapter_id })
      .populate("CapsuleId", "MetaData Title")
      .populate("OwnerId", "Email Name ProfilePic")
      .exec(function (err, chapterData) {
        if (err) {
          res.json({ status: 500, message: "Server error", error: err });
        } else {
          res.json({
            status: 200,
            msg: "Chapter Data got",
            results: chapterData,
          });
        }
      });
  } else if (req.body.page_id) {
    //Journal Module Integration
    Page.findOne(
      { _id: req.body.page_id },
      { Medias: 0 },
      function (err, chapterData) {
        if (err) {
          res.json({ status: 500, message: "Server error", error: err });
        } else {
          res.json({
            status: 200,
            msg: "Page data fetched successfully!",
            results: chapterData,
          });
        }
      }
...
exports.invitationForMe__getLaunchedChapters =
...
exports.allByCapsuleId__getLaunchedChapters =
  allByCapsuleId__getLaunchedChapters; //For Verify and Public Gallery Capsule listing
