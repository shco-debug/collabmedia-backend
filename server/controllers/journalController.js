	...
	aggregateStages.push ({
		$project : {
			...
			"value" : {
				...
				MediaSelectionCriteriaCount: {$size : { $ifNull: ["$MediaSelectionCriteriaArr", [] ] }}
			}
		}
	});
...
var getMediaFromSet2 = async function (req, callback) {
	...
	if(selectedKeywords.length) {
		...
		if(switchBranches.length) {
			...
			//logic for Media Seletion criteria case
			reqObj.MediaSelectionCriteria = {
				PrimaryBrandArchetype: 'Wise Sage',
				SecondaryBrandArchetype: 'Transformational Magician',
				TertiaryBrandArchetype: 'Strategic Thinker',
				MBTI: 'INFJ'
			};
			let mediaSelectionCriteria = reqObj.mediaSelectionCriteria || null;
			if(mediaSelectionCriteria) {
				for (const property in mediaSelectionCriteria) {
					console.log(`${property}: ${mediaSelectionCriteria[property]}`);
					groupObj["$group"]["MediaSelectionCriteriaArr"] = {
						$push: {
							$cond:[
								{ $eq: [`$MetaData.${property}`, `${mediaSelectionCriteria[property]}`] },
								1,
								"$$REMOVE"
							]
						}
