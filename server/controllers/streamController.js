...
async function __getKeywordIdsByNames (selectedWords) {
	var selectedWords = selectedWords ? selectedWords : [];

	for(var i = 0; i < selectedWords.length; i++) {
		selectedWords.push(selectedWords[i].trim());
	}
	var conditions = {
		status : { $in : [1, 3] },
...
async function getMyStreamIds (UserId) {
	var conditions = {
		"$or" : [
			{
				...
				"LaunchSettings.StreamType": {$ne : "Group"}
			}
		],
		Status : true,
		IsDeleted : false
	...
	var fields = {_id: 1, Order: 1, ModifiedOn: -1};
	var sortObj = {
		_id : 1,
		Order: 1,
		ModifiedOn: -1
...
var getMediaFromSet = async function (req, callback) {
	...
	var conditions = {
		...
		UploadedBy : "admin",
		$or : [{ MediaType : "Link", LinkType : "image", IsUnsplashImage : {$exists : true}, IsUnsplashImage : true }],
		InAppropFlagCount : { $lt:5 }
	};
	...
	aggregateStages.push ({
		$project : {
			...
			"value" : {
				...
				//SecondaryKeywordsCount : { $size: "$SecondaryKeywords" }
...
var streamPost_withEmailSync_v2 = function (req,res) {
	req.session = req.session ? req.session : {};
	var PostImage = req.body.PostImage ? req.body.PostImage : "";
	var PostStatement = req.body.PostStatement ? req.body.PostStatement : "";
...
var streamPage__WithSelectedBlendCase = function (req, res) {
	req.session = req.session ? req.session : {};
	var PostImage = req.body.PostImage ? req.body.PostImage : "";
	var PostStatement = req.body.PostStatement ? req.body.PostStatement : "";
...
var updateLightnessScore = function (req, res) {
	var inputArr = [
        {
            "_id": "5c5bbdb7d960ee4bdefc3ca5",
            "Lightness": "0.1686",
            "DominantColors": "#29221b, #a8a9b4, #726665, #504440, #59575e, #9a806d, #434442, #8a8c88, #5b5c4e"
        },
        {
            "_id": "553e56cd880974bb503f11bc",
            "Lightness": "0.1673",
            "DominantColors": "#282b2d, #d8e3db, #595e64, #737b7e, #a4b1ad, #8f9898, #95a49b, #4b5c58, #5e6c63"
        }
    ];
	return;
	for(var i = 0; i < inputArr.length; i++) {
		...
		var setObj = {
			...
			DominantColors : mediaObj.DominantColors ? mediaObj.DominantColors : "",
		}
		var options = { multi : false };
		Media.update(conditions, {$set : setObj}, options, function(){
...
var generatePostBlendImage = function(req, res) {
	try {
		var obj = req.body;
	...
	setTimeout(function(){
		return res.json({code : 200, message : 'Blend Image generated successfully', status: 'Generating'});
	}, 3000);
}

var generatePostBlendImage_INTERNAL_API = function(req, res) {
	try {
		var obj = req.body;
...
var myStreamPosts = async function (req, res) {
	req.body.pageNo = req.body.pageNo || 1;
	req.body.pageNo = req.body.pageNo - 1;
	req.body.perPage = req.body.perPage || 50;
	...
	var IsAllStream = req.body.IsAllStream ? req.body.IsAllStream : false;
	...
	if(StreamType == 'Group' && String(OwnerId) != String(loginUserId)) {
		...
		if(StreamType == 'Group') {
			...
			if(String(req.session.user._id) == "5f4835e67a5d1b040c63237e" || tmp_emails.indexOf(req.session.user.Email.toLowerCase()) >= 0) { //added on 26th July 2023
				sortBy = {"EmailEngineDataSets.DateOfDelivery" : 1};
			}
		}

		if(IsAllStream && StreamType != 'Group') {
			const allStreamIds = await getMyStreamIds(req.session.user._id);
			delete conditions.CapsuleId;
			delete finalConditions.CapsuleId;
			conditions.CapsuleId = {$in : allStreamIds};
			finalConditions.CapsuleId = {$in : allStreamIds};
		}
	...
	try {
		...
		for ( let loop = 0; loop < syncedPostsResults.length; loop++ ) {
						...
						var CapsuleData = {
							...
							Title: dataRecord.CapsuleData.Title ? dataRecord.CapsuleData.Title : ''
						};
...
var myStreamPosts_friendAsOwner = async function (req, res) {
	...
	SyncedPost.aggregate([
		...
		{ $sort : {"EmailEngineDataSets.DateOfDelivery" : -1} },
		{ $skip : (req.body.pageNo * req.body.perPage) },
		{ $limit : req.body.perPage },
		{
			$project: {
	...
	SyncedPost.aggregate([
		...
		if ( !err ) {
			...
			try {
				...
				req.body.totalRecords = req.body.totalRecords || 0;
				return res.json({"code":"200", message : "success", results: streamPosts, stage : "2", totalRecords: req.body.totalRecords});
			} catch (error) {
				console.log("error --------- ", error);
				return res.json({"code":"501", message : "Something went wrong.", caughtError: error, results : []});
			}
...
var addCommentOnSocialPost = async function(req, res) {
	...
	if (req.body.SocialPostId) {
		try {
			...
			console.log('✅ [streamController] Valid SocialPostId (SyncedPost._id):', socialPostId);
		} catch (e) {
			console.error('❌ [streamController] Invalid SocialPostId format:', req.body.SocialPostId);
		...
		console.error('❌ [streamController] Missing SocialPostId (SyncedPost._id)');
		return res.status(400).json({
			code: 400,
	...
	console.log('💾 [streamController] Saving comment with data:', JSON.stringify({
...
var addLike = async function(req, res) {
	...
	// ✅ CRITICAL: CommentId MUST be StreamComments._id (works for both comments and replies)
	// Replies are also StreamComments documents with a ParentId field
	...
	try {
		...
		if (!CommentId || !SocialPageId) {
			return res.json({
				status : "error",
				message : "CommentId (StreamComments._id) and SocialPageId are required.",
				results : []
			});
		}
		...
		if (!ObjectId.isValid(CommentId) || !ObjectId.isValid(SocialPageId)) {
			return res.json({
				status : "error",
				message : "Invalid CommentId (StreamComments._id) or SocialPageId format.",
				results : []
			});
		}

		// Check if the comment/reply exists in StreamComments collection
		// ✅ Note: This works for both top-level comments and replies (replies are StreamComments with ParentId)
		...
		if (!commentExists) {
			return res.json({
				status : "error",
				message : "Comment/Reply not found or has been deleted.",
				results : []
			});
		}

		// Check if user already liked this comment (check regardless of IsDeleted status)
		...
		if (existingLike) {
			...
			if (existingLike.IsDeleted === 0 || existingLike.IsDeleted === false) {
				...
				// ✅ Like exists but was deleted - restore it (reuse existing entry)
				existingLike.IsDeleted = false;
				existingLike.ModifiedOn = new Date();
				const savedLike = await existingLike.save();
		...
		// No existing like found - create new entry
		// ✅ CommentId is StreamComments._id (works for both comments and replies)
	var dataToSave = {
			SocialPageId : new ObjectId(SocialPageId),
			CommentId : new ObjectId(CommentId), // ✅ StreamComments._id (comment or reply)
			LikedById : new ObjectId(loginUserId),
			CreatedOn : new Date(),
			ModifiedOn : new Date(),
			IsDeleted : false
