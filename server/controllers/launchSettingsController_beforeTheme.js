		...
		if (!err) {
			...
			if (myCart.CartItems.length > 0) {
				...
				if (order.CartItems.length > 0) {
					...
					// console.log("---------------order.TotalPayment-----------", order.TotalPayment);
					// //console.log("---------------order.TotalPlatformCommission-----------", order.TotalPlatformCommission);

					// console.log("---------------(order.TotalPayment*100)-----------", (order.TotalPayment * 100));
					// return;
					// //console.log("---------------parseInt(order.TotalPayment*100)-----------", parseInt(order.TotalPayment * 100));
						...
						if (results.length) {
							...
							if (cartItemOwners.length) {
								for (var loop = 0; loop < cartItemOwners.length; loop++) {
									...
									//var UniqueIdPerOwner = UniqueIds[loop] ? UniqueIds[loop] : null;
									capsule__createNewInstance(CapsuleData, owner, req, loop);
								}
							}
							else {
								var response = {
...
function __checkNcreateChapterIntroPage(data, shareWithName, CapsuleData) {
	...
	if(OriginatedFromChapterId) {
		var chapterIntroPagePath = __dirname + "/../../public/views/intro-pages/" + String(OriginatedFromChapterId) + ".html";
		...
		//Owner's case ...
		var owner_chapterIntroPagePath = __dirname + "/../../public/views/intro-pages/" + String(OriginatedFromChapterId) + "_O.html";
