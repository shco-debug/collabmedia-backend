																		...
																		if (!err) {
																			...
																			if(CapsuleData.LaunchSettings.CapsuleFor == 'Stream' && CapsuleData.LaunchSettings.StreamType != 'Group') {
																				if(data.Medias.length) {
																					...
																					__streamPagePostNow(data.Medias, result, shareWithEmail, req, CapsuleData);
																				} else {
...
var savedOrder = function (charge, orderInit, payFrom, rdm_credit, result, order, req, res) {
	...
	if (charge.paid && charge.failure_code == null || payFrom == 'free') {
		...
		for (var loop = 0; loop < order.CartItems.length; loop++) {
			...
			capsule__createNewInstances_perCartItem(__capsuleId, cartItemOwners, cartItem, function (err, result) {
				totalOps++;
				//if( !err ){
			...
			function capsule__createNewInstances_perCartItem(__capsuleId, cartItemOwners, cartItem, callback) {
				var conditions = {
					_id: __capsuleId
				}
				var fields = {};
