...
async function fetchKeywordsFromText_PrimarySecondary(text) {
                    ...
                    contents: [
                        {
                            ...
                            parts: [
                                {
                                    text: "You are an accurate researcher.\n" + prompt + '\n' + inputText
                                }
                            ]
                        }
                    ],
    ...
    var primaryKeywords = keywordPartsArr.length >= 1 ? keywordPartsArr[0].split(',').slice(0, 2).map((obj) => obj.trim()) : [];
    var secondaryKeywords = keywordPartsArr.length >= 2 ? keywordPartsArr[1].split(',').map((obj) => obj.trim()) : [];
    var secondary2Keywords = keywordPartsArr.length === 3 ? keywordPartsArr[2].split(',').map((obj) => obj.trim()) : [];
    ...
    keywordResult.Primary = Array.isArray(keywordResult.Primary) ? keywordResult.Primary : [];
    keywordResult.Secondary = Array.isArray(keywordResult.Secondary) ? keywordResult.Secondary : [];
    keywordResult.Secondary2 = Array.isArray(keywordResult.Secondary2) ? keywordResult.Secondary2 : [];
...
var searchMedia = async function (req, res) {
	...
	var mediaCount = await Media.count(conditions).exec();
...
var getMediaFromSet = async function (req, callback) {
	...
	console.log("Media model is:", Media);
...
var getMediaFromSet2 = async function (req, callback) {
	console.error('ENTER getMediaFromSet2');
...
var myStreamPosts = async function (req, res) {
	...
	if(StreamType == 'Group' && String(OwnerId) != String(loginUserId)) {
		...
		if(StreamType == 'Group') {
			...
			if(String(req.session.user._id) == "5f4835e67a5d1b040c63237e" || tmp_emails.indexOf(req.session.user.Email.toLowerCase()) >= 0) {
...
var addNewPost_INTERNAL_API = async function (req, res) {
	...
	try {
		...
		console.log("44444444444444444444------------keywordsObj = ", keywordsObj);
		var keywords = keywordsObj.Primary || [];
		var secondaryKeywords = keywordsObj.Secondary || [];
