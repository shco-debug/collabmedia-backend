...
async function fetchKeywordsFromText_rnd(text) {
  ...
  if (inputText) {
    try {
      ...
      var request_url =
        "http://yake.inesctec.pt/yake/v2/extract_keywords?max_ngram_size=1&number_of_keywords=20&highlight=false";
...
async function fetchKeywordsFromText(text) {
  ...
  if (inputText) {
    ...
    var request_url =
      ...
      encodeURI(inputText) +
      "&max_ngram_size=1&number_of_keywords=20&highlight=false";
...
async function fetchKeywordsFromText_PrimarySecondary_PythonAPI(text) {
  ...
  keywordResult.Primary = Array.isArray(keywordResult.Primary)
    ...
    : [];
  keywordResult.Secondary = Array.isArray(keywordResult.Secondary)
...
async function getPrimarySecondaryKeywordsPrompt() {
  console.log("🔍 DEBUG: getPrimarySecondaryKeywordsPrompt called");
  console.log("   - AppSettings model:", typeof AppSettings);
  console.log("   - AppSettings available:", !!AppSettings);
  
  try {
    console.log("   - Querying AppSettings collection...");
    
    // First, let's check if we can query the collection at all
    console.log("   - Testing basic AppSettings query...");
    let testResult = await AppSettings.find({}).limit(1);
    console.log("   - Basic query result:", testResult);
    console.log(
      "   - Basic query result length:",
      testResult ? testResult.length : 0
    );
    
    let result = await AppSettings.findOne({
      PrimarySecondaryKeywordsPrompt: { $exists: true },
    });
    console.log("   - Raw result from AppSettings:", result);
    ...
    console.log(
      "   - PrimarySecondaryKeywordsPrompt exists:",
      !!result.PrimarySecondaryKeywordsPrompt
    );
    
    if (result.PrimarySecondaryKeywordsPrompt) {
      console.log(
        "   - Using stored prompt:",
        result.PrimarySecondaryKeywordsPrompt
      );
    ...
    console.log(
      "❌ ERROR in getPrimarySecondaryKeywordsPrompt:",
      error.message
    );
...
const updatePrimarySecondaryKeywordsPrompt_BROWSER_API = async function (
  req,
  res
) {
  const PrimarySecondaryKeywordsPrompt =
    req.query.PrimarySecondaryKeywordsPrompt ||
    req.body.PrimarySecondaryKeywordsPrompt ||
    null;
  if (!PrimarySecondaryKeywordsPrompt) {
    return res.json({ 
      code: 501, 
      message:
        "Wrong input. Please provide PrimarySecondaryKeywordsPrompt as query parameter or in request body.",
      example:
        "GET /journal/updatePrimarySecondaryKeywordsPrompt_BROWSER_API?PrimarySecondaryKeywordsPrompt=YOUR_PROMPT_HERE",
    });
  }

  console.log("🔍 DEBUG: About to update AppSettings collection");
  console.log("   - Query filter:", {
    PrimarySecondaryKeywordsPrompt: { $exists: true },
  });
  console.log("   - Update data:", {
    $set: { PrimarySecondaryKeywordsPrompt: PrimarySecondaryKeywordsPrompt },
  });
  
  try {
    const updateResult = await AppSettings.updateOne(
      { PrimarySecondaryKeywordsPrompt: { $exists: true } },
      {
        $set: {
          PrimarySecondaryKeywordsPrompt: PrimarySecondaryKeywordsPrompt,
        },
      }
    );
    ...
    if (updateResult.matchedCount === 0) {
      ...
      const insertResult = await AppSettings.create({
        PrimarySecondaryKeywordsPrompt: PrimarySecondaryKeywordsPrompt,
        StreamMediaFilterSortingOrder: "231",
        ReferralDiscount: 0,
        isDeleted: false,
      });
    ...
    return res.json({
      code: 200,
      message: "PrimarySecondaryKeywordsPrompt has been updated successfully",
    });
  } catch (error) {
    console.log("❌ ERROR updating AppSettings:", error.message);
    console.log("   - Error stack:", error.stack);
    return res.json({
      code: 500,
      message: "Failed to update PrimarySecondaryKeywordsPrompt",
      error: error.message,
    });
  }
};

const getPrimarySecondaryKeywordsPrompt_BROWSER_API = async function (
  req,
  res
) {
  return res.json({
...
async function getStreamMediaFilterSortingOrder() {
  ...
  result = typeof result === "object" ? result : {};
  let StreamMediaFilterSortingOrder =
    result.StreamMediaFilterSortingOrder || "231";
...
const updateStreamMediaFilterSortingOrder_BROWSER_API = async function (
  req,
  res
) {
...
async function fetchKeywordsFromText_PrimarySecondary(text) {
  ...
  if (inputText) {
    inputText = inputText.replace(/[^a-zA-Z0-9: ]/g, "");
    console.log("   - Input text cleaned:", inputText);
    console.log("   - About to get prompt...");
    ...
    try {
      const geminiResponse = await axios.post(
        GEMINI_URL, // Use the dynamically generated URL
        {
          contents: [
            {
              role: "user",
        ...
        },
        {
          headers: {
            ...
            "x-goog-api-key": "AIzaSyCAFqkuBP5nzSA_93wEfQ7zOD4BMOm4vO8",
          },
          params: {
            key: "AIzaSyCAFqkuBP5nzSA_93wEfQ7zOD4BMOm4vO8",
          },
        }
      );
      ...
      console.log("Caught Error from Gemini API = ", err);
    }
  }
  ...
  keywordResult.Secondary = Array.isArray(keywordResult.Secondary)
    ? keywordResult.Secondary
    : [];
  keywordResult.Secondary2 = Array.isArray(keywordResult.Secondary2)
    ? keywordResult.Secondary2
    : [];
...
async function fetchKeywordsFromTextByNgram_Backup(text, ngram) {
  ...
  if (inputText) {
    ...
    var request_url =
      ...
      "&max_ngram_size=" +
      ngram +
      "&number_of_keywords=20&highlight=false";
    response = await axios.get(request_url);
...
async function fetchKeywordsFromTextByNgram(text, ngram) {
  ...
  if (inputText) {
    ...
    console.log(
      "axios http://www.scrpt.com:5002/api/extractv1_10grams phrase Result = ",
      keywordResult
    );
  }
  return keywordResult;
...
async function getPageIdByStream(StreamId) {
  var ChapterId = null;
  var PageId = null;
  var chapterResult = await Chapter.findOne(
    { CapsuleId: StreamId, IsDeleted: false },
    { _id: 1 }
  );
  ...
  var pageResult = {};
  if (ChapterId) {
    pageResult = await Page.findOne(
      { ChapterId: ChapterId, IsDeleted: false, IsDasheditpage: false },
      { _id: 1 }
    );
...
async function getStreamDataByChapterId(ChapterId) {
  var CapsuleId = null;
  var chapterResult = await Chapter.findOne(
    { _id: new ObjectId(ChapterId), IsDeleted: false },
    { CapsuleId: 1 }
  );
  chapterResult = typeof chapterResult == "object" ? chapterResult : {};
  ...
  if (CapsuleId) {
    streamResult = await Capsule.findOne({
...
async function getPreLaunchPostsByStreamId(StreamId) {
  ...
  var PageId = null;
  var chapterResult = await Chapter.findOne(
    { CapsuleId: String(StreamId), IsDeleted: false },
    { _id: 1 }
  );
  chapterResult = typeof chapterResult == "object" ? chapterResult : {};
  ChapterId = chapterResult._id ? chapterResult._id : null;
  ...
  if (ChapterId) {
    ...
    posts = Array.isArray(pageResult)
      ? pageResult.length
        ? pageResult[0].posts
        : []
      : [];
...
async function notifyMembers(users, UserName, Action, StreamId) {
  ...
  for (var i = 0; i < users.length; i++) {
    ...
    memberObj.PostActionsNotification = memberObj.PostActionsNotification
      ? memberObj.PostActionsNotification
      : false;
    ...
    newHtml = newHtml.replace(
      /{SubscriberId}/g,
      CommonAlgo.commonModule.strToCustomHash(memberObj.Email)
    );
...
var __updateChapterCollection = function (
  registeredUserEmail,
  registeredUserId
) {
  if (registeredUserEmail && registeredUserId) {
    ...
    Capsule.updateMany(
      ...
      // Capsule ownership transfer completed
    }).catch(function (err) {
      // Capsule ownership transfer error
    });
    ...
    Chapter.updateMany(
      ...
      // Chapter ownership transfer completed
    }).catch(function (err) {
      // Chapter ownership transfer error
    });
    ...
    Page.updateOne(
      ...
      function (err, numAffected) {
        // Page ownership transfer completed or error
      }
    );
  } else {
    // Missing required parameters for ownership transfer
  }
};

//updates for invitations case
var __updateChapterCollection__invitationCase = function (
  registeredUserEmail,
  registeredUserId
) {
  if (registeredUserEmail && registeredUserId) {
    ...
    Chapter.updateMany(
      ...
      // Chapter invitation update completed
    }).catch(function (err) {
      // Chapter invitation update error
    });
  } else {
    // Missing required parameters for invitation update
  }
};
...
var __createDefaultJournal_BackgroundCall = async function (user_id, user_email) {
  ...
  try {
    ...
    if (result) {
      ...
      try {
        await User.updateOne(updateCond, dataToSet);
      ...
      for (var loop = 0; loop < myDefaultFolders.length; loop++) {
        data.Title = myDefaultFolders[loop]
          ? myDefaultFolders[loop]
          : "Untitled Folder";

        try {
          var chapterResult = await Chapter(data).save();
          if (chapterResult) {
            //save All Folders - id in users collection...
            if (chapterResult.Title == "General") {
              var updateCond = {
                _id: user_id,
              };
              var dataToSet = {
                AllFoldersId: String(chapterResult._id),
              };
              try {
                await User.updateOne(updateCond, dataToSet);
                // AllFoldersId updated successfully
              } catch (err) {
                // AllFoldersId update error
              }
              ...
              data.Labels = [
                {
                  LabelId: new ObjectId("5ff179065a0a9a452c791f54"),
                  Label: "Goal",
                  Icon: "Goal.png",
                  AddedBy: "Admin",
                },
                {
                  LabelId: new ObjectId("5ff179255a0a9a452c791f56"),
                  Label: "Old story",
                  Icon: "Old_story.png",
                  AddedBy: "Admin",
                },
                {
                  LabelId: new ObjectId("5ff179465a0a9a452c791f58"),
                  Label: "New story",
                  Icon: "New_story.png",
                  AddedBy: "Admin",
                },
                {
                  LabelId: new ObjectId("5ff179555a0a9a452c791f59"),
                  Label: "Actions",
                  Icon: "Skillful_action.png",
                  AddedBy: "Admin",
                },
                {
                  LabelId: new ObjectId("5ff1797f5a0a9a452c791f5d"),
                  Label: "Wins",
                  Icon: "Victory.png",
                  AddedBy: "Admin",
                },
              ...
              try {
                var pageResult = await Page(data).save();
                if (pageResult) {
                  //save General page - id in users collection...
                  if (pageResult.Title == "General") {
                    var updateCond = {
                      _id: user_id,
                    };
                    var dataToSet = {
                      AllPagesId: String(pageResult._id),
                    };
                    try {
                      await User.updateOne(updateCond, dataToSet);
                    // AllPagesId updated successfully
                  } catch (err) {
                    // AllPagesId update error
                  }
                  }
                  //save General page - id in users collection...
              ...
              data.ChapterId = chapterResult._id ? chapterResult._id : null;
              data.PageType = "gallery";
              ...
              for (
                var loop2 = 0;
                loop2 < myDefaultFoldersPages.length;
                loop2++
              ) {
                data.Title = myDefaultFoldersPages[loop2]
                  ? myDefaultFoldersPages[loop2]
                  : "Untitled Page";
                try {
                  var pageResult2 = await Page(data).save();
                  if (pageResult2) {
                  var response = {
                    status: 200,
                    message: "Page created successfully.",
  ...
  try {
    ...
    var fields = {};
    var results = await Page.find(conditions, fields);
    if (results) {
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        for (var j = 0; j < result.Medias.length; j++) {
          result.Medias[j].OwnerId = user_id;
          result.Medias[j].PostedBy = new ObjectId(user_id);
        }

        try {
          var pageResult = await Page(result).save();
          if (pageResult) {
...
var getMyFolders = function (req, res) {
  ...
  async_lib.parallel(
    ...
    function (err, results) {
      ...
      if (!err) {
        var allInvitedChaptersIds = results.getAllInvitedChapters.length
          ? results.getAllInvitedChapters
          : [];
        var allPurchasedChapterIds = results.getAllPurchasedChapters.length
          ? results.getAllPurchasedChapters
          : [];
        var allVisitedPublicChapters = results.getAllVisitedPublicChapters
          ? results.getAllVisitedPublicChapters
          : [];

        for (var i = 0; i < allInvitedChaptersIds.length; i++) {
          for (var j = 0; j < allVisitedPublicChapters.length; j++) {
            if (
              String(allInvitedChaptersIds[i]) ==
              String(allVisitedPublicChapters[j])
            ) {
        ...
        Chapter.find(conditions, fields)
          ...
          .exec(function (err, results) {
            if (!err) {
              ...
              var publicTopicsArr = [];
              for (var loop = 0; loop < results.length; loop++) {
                var OwnerName = results[loop].OwnerId.Name
                  ? results[loop].OwnerId.Name
                  : "";
                results[loop].OwnerId = String(results[loop].OwnerId._id);
                var record = results[loop];
                if (
                  !record.IsPublic &&
                  record.OwnerId != req.session.user._id
                ) {
                  results[loop].Title =
                    results[loop].Title + " (" + OwnerName.split(" ")[0] + ")";
...
var createFolder = function (req, res) {
  ...
  var journalId = req.session.user.JournalId
    ? req.session.user.JournalId
    : null;
  var data = {};
  ...
  if (!req.body.editFolderId) {
    ...
    Chapter.find(uniqueCondition)
      .count()
      .exec(function (err, ChapterCount) {
        if (!err) {
          if (!ChapterCount) {
            Chapter(data).save(function (err, result) {
              if (!err) {
...
// ES6_COPIED_TO_newJournalControllerForV2.js - Function createPage has been copied and modernized
var createPage = function (req, res) {
  ...
  for (var loop = 0; loop < req.body.Themes.length; loop++) {
    ...
    if (!gtId) {
      req.body.Themes[loop].text = req.body.Themes[loop].text
        ? req.body.Themes[loop].text
        : "";
      req.body.Themes[loop].text = req.body.Themes[loop].text.replace(
        /-/g,
        " "
      );
    }
      ...
      chkUserGt(userSuggestedGt.text, function (response) {
        ...
        if (response.length >= 1) {
          ...
          for (var loop = 0; loop < postThemeArr.length; loop++) {
            ...
            if (
              !gtId &&
              postThemeArr[loop].text.toLowerCase() ==
                response[0].GroupTagTitle.toLowerCase()
            ) {
            ...
            if (err) {
              ...
              for (var loop = 0; loop < postThemeArr.length; loop++) {
                ...
                if (
                  !gtId &&
                  postThemeArr[loop].text.toLowerCase() ==
                    data.GroupTagTitle.toLowerCase()
                ) {
  ...
  function NextStepAfterUserSuggestedTagMapping() {
    if (req.body.editPageId) {
      ...
      if (folderId) {
        ...
        Page.updateOne(
          { _id: data._id },
          { $set: { Title: data.Title, Themes: data.Themes } },
          function (err, result) {
            if (!err) {
              var response = {
                status: 200,
...
// ES6_COPIED_TO_newJournalControllerForV2.js - Function chkUserGt has been copied and modernized
function chkUserGt(abc, callback) {
  ...
  if (abc) {
    groupTags.find(
      {
        status: { $in: [1, 2, 3] },
        GroupTagTitle: { $regex: new RegExp("^" + abc + "$", "i") },
      },
      function (err, result) {
        if (err) {
          throw err;
          ...
          if (!result.length) {
            groupTags.find(
              {
                status: { $in: [1, 2, 3] },
                "Tags.TagTitle": { $regex: new RegExp("^" + abc + "$", "i") },
                "Tags.status": 1,
              },
    ...
    .exec(function (err, results) {
      if (!err) {
        ...
        for (var loop = 0; loop < results.length; loop++) {
          var record = results[loop];
          results[loop].Title =
            results[loop].ChapterId.Title + " > " + results[loop].Title;

          if (record.OwnerId != req.session.user._id) {
            results[loop].Title =
              results[loop].Title +
              " (" +
              record.OwnerId.Name.split(" ")[0] +
              ")";
            secondArr.push(results[loop]);
          } else {
            firstArr.push(results[loop]);
  ...
  if (label && page_id) {
    chkLabel(label, function (response) {
      ...
      if (response.length > 1) {
        ...
        Page.updateOne(conditions, dataToUpdate, function (err, result) {
          if (!err) {
            res.json({
              code: "200",
              message: "label saved",
...
var deleteLabel = function (req, res) {
  ...
  if (label_id && page_id) {
    ...
    Page.updateOne(conditions, dataToUpdate, function (err, data) {
      if (!err) {
...
var syncPost = function (req, res) {
  var BlendMode = "overlay"; // Default blend mode
  ...
  dataRecord.ReceiverEmails =
    typeof dataRecord.ReceiverEmails == "object"
      ? dataRecord.ReceiverEmails
      : [];
  ...
  if (
    dataRecord.PageId &&
    dataRecord.PostId &&
    dataRecord.PostOwnerId &&
    dataRecord.ReceiverEmails.length
  ) {
          ...
          if (!err) {
            if (results.length) {
              var SharedByUserName = req.session.user.Name
                ...
                : "";
              var PostURL =
                "https://www.scrpt.com/post_view?post=" + dataRecord.PostId;
                ...
                function (err, UserData) {
                  if (!err) {
                    ...
                    for (var i = 0; i < UserData.length; i++) {
                      var RecipientName = UserData[i].Name
                        ? UserData[i].Name.split(" ")[0]
                        : "";
                      var shareWithEmail = UserData[i].Email
                        ? UserData[i].Email
                        : null;
                    ...
                    if (emails.length != dataRecord.ReceiverEmails.length) {
                      var difference = dataRecord.ReceiverEmails.filter(
                        (x) => emails.indexOf(x) === -1
                      );
                      for (var i = 0; i < difference.length; i++) {
                        var RecipientName = difference[i]
                          ? difference[i].split("@")[0]
                          : "";
                        var shareWithEmail = difference[i]
                          ? difference[i]
                          : null;
...
var sendSurprisePost = function (req, res) {
  var SurpriseSelectedTags = req.body.SurpriseSelectedTags
    ? req.body.SurpriseSelectedTags
    : [];

  if (SurpriseSelectedTags.length < 2) {
    return res.json({
      code: "205",
      message: "No selected keywords found for blending ...",
    });
  }
  ...
  Media.aggregate(
    [
      {
        $match: {
          "GroupTags.GroupTagTitle": { $in: SurpriseSelectedTags },
          MetaMetaTags: {
    ...
    function (err, results) {
      ...
      if (results.length == 1) {
        ...
        BlendImage1 =
          results[0].Medias[
            Math.floor(Math.random() * results[0].Medias.length)
          ];
        BlendImage2 =
          results[0].Medias[
            Math.floor(Math.random() * results[0].Medias.length)
        ...
        SurpriseImagesFrequencyWise.push({
          BlendImage1: BlendImage1.MediaURL
            ? BlendImage1.MediaURL
            : "https://www.scrpt.com/assets/Media/img/600/" +
              BlendImage1.MediaURL2[0].URL,
          BlendImage2: BlendImage2.MediaURL
            ? BlendImage2.MediaURL
            : "https://www.scrpt.com/assets/Media/img/600/" +
              BlendImage2.MediaURL2[0].URL,
        });
        ...
        BlendImage2 =
          results[1].Medias[
            Math.floor(Math.random() * results[1].Medias.length)
          ];
        ...
        return res.json({
          code: "205",
          message: "No images found for blending ...",
        });
      }
      ...
      var PostImage = req.body.PostImage ? req.body.PostImage : "";
      var PostImage1 = SurpriseImagesFrequencyWise[0].BlendImage1
        ? SurpriseImagesFrequencyWise[0].BlendImage1
        : PostImage;
      var PostImage2 = SurpriseImagesFrequencyWise[0].BlendImage2
        ? SurpriseImagesFrequencyWise[0].BlendImage2
        : PostImage;
      var PostStatement = req.body.PostStatement ? req.body.PostStatement : "";
...
// Helper function to get blend settings from Media collection
async function getBlendSettingsFromMedia(mediaId) {
  try {
    const media = await Media.findOne({ _id: mediaId }, { BlendSettings: 1 });
    if (media && media.BlendSettings) {
      return {
        blendMode: media.BlendSettings.blendMode || "overlay",
        image1Url: media.BlendSettings.image1Url || "",
        image2Url: media.BlendSettings.image2Url || "",
        keywords: media.BlendSettings.keywords || [],
        selectedKeywords: media.BlendSettings.selectedKeywords || []
      };
    }
  } catch (error) {
    console.log("❌ Error retrieving blend settings:", error.message);
  }
  return {
    blendMode: "overlay",
    image1Url: "",
...
var sendSurprisePost_withEmailSync = function (req, res) {
  ...
  var SurpriseSelectedTags = req.body.SurpriseSelectedTags
    ? req.body.SurpriseSelectedTags
    : [];
  var EmailEngineDataSets = req.body.EmailEngineDataSets
    ? req.body.EmailEngineDataSets
    : [];
      ...
      if (results.length == 1) {
        ...
        for (var i = 0; i < EmailEngineDataSets.length; i++) {
          ...
          EmailEngineDataSets[i].DateOfDelivery =
            getDateIncrementedBy(NoOfDays);
          EmailEngineDataSets[i].VisualUrls = EmailEngineDataSets[i].VisualUrls
            ? EmailEngineDataSets[i].VisualUrls
            : [];
          EmailEngineDataSets[i].TextAboveVisual = EmailEngineDataSets[i]
            .TextAboveVisual
            ? EmailEngineDataSets[i].TextAboveVisual
            : "";
          EmailEngineDataSets[i].TextBelowVisual = EmailEngineDataSets[i]
            .TextBelowVisual
            ? EmailEngineDataSets[i].TextBelowVisual
            : "";
          EmailEngineDataSets[i].SoundFileUrl = EmailEngineDataSets[i]
            .SoundFileUrl
            ? EmailEngineDataSets[i].SoundFileUrl
            : null;
          if (!EmailEngineDataSets[i].VisualUrls.length) {
            ...
            EmailEngineDataSets[i].VisualUrls.push(
              BlendImage1.MediaURL
                ? BlendImage1.MediaURL
                : "https://www.scrpt.com/assets/Media/img/600/" +
                    BlendImage1.MediaURL2[0].URL
            ...
            EmailTemplate.find(condition, {}, function (err, results) {
              if (!err) {
                if (results.length) {
                  ...
                  if (SoundFileUrl) {
                    SoundFileUrl =
                      '<p style="display: block;"><span style="font-size: 18px;"><br></span></p><p style="clear: both;display: block;"><em style="font-size: 18px; background-color: transparent;"><audio controls><source src="' +
                      SoundFileUrl +
                      '" type="video/mp4">Your email does not support the audio.</audio></em></p>';
...
var sendPost = function (req, res) {
  ...
  req.body.IsSurpriseCase = req.body.IsSurpriseCase
    ? req.body.IsSurpriseCase
    : false;
...
const updatePostLabelId = async function (req, res) {
  try {
    const PostId = req.body.post_id ? req.body.post_id : null;
    const LabelId = req.body.label_id ? req.body.label_id : null;
    ...
    const conditions = {
      "Medias._id": new ObjectId(PostId),
      IsDasheditpage: false,
    };
    ...
    if (LabelId) {
      setObj = {
        $set: {
          "Medias.$.Label": new ObjectId(LabelId),
        },
      };
    } else {
      setObj = {
    ...
    // Update posts in non-dashedit pages
    const result = await Page.updateOne(conditions, setObj, options);
    
    if (result.modifiedCount === 0) {
      console.log("No posts updated in non-dashedit pages");
    }

    // Update posts in dashedit pages
    const conditions2 = {
      "Medias._id": new ObjectId(PostId),
      IsDasheditpage: true,
    };

    const result2 = await Page.updateOne(conditions2, setObj, options);
    
    if (result2.modifiedCount === 0) {
      console.log("No posts updated in dashedit pages");
    }
    ...
    console.log("Error in updatePostLabelId:", err);
    const response = {
      status: 500,
      message: "Something went wrong.",
      error: err.message,
                        ...
                        Page.findOne(
                          ...
                          function (err, result) {
                            ...
                            data.HeaderBlurValue = result.HeaderBlurValue
                              ...
                              : 0;
                            data.HeaderColorCode = result.HeaderColorCode
                              ? result.HeaderColorCode
                              : null;
                            ...
                            data.IsLabelAllowed = result.IsLabelAllowed
                              ? result.IsLabelAllowed
                              : false;
                            data.HeaderVideoLink = result.HeaderVideoLink
                            ...
                            data.LaunchSettings = {
                              ...
                              Invitees: [
                                {
                                  UserPic: req.session.user.ProfilePic
                                    ...
                                    function (err1, data2) {
                                      if (!err) {
                                        if (data2.length > 0) {
                                          ...
                                          newFriendData2.ID =
                                            req.session.user._id;
                                          newFriendData2.Email =
                                            req.session.user.Email;
                                          newFriendData2.Name =
                                            req.session.user.Name;
                                          newFriendData2.NickName =
                                            req.session.user.NickName;
                                          newFriendData2.Pic =
                                            req.session.user.ProfilePic;
                                          newFriendData2.Relation =
                                            rel[0].trim();
                                          newFriendData2.RelationID =
                                            rel[1].trim();
    ...
    [
      ...
      { $sort: { "Medias.PostedOn": -1 } },
      {
        $group: {
          _id: "$_id",
          Medias: {
            $push: { _id: "$Medias._id", PostedOn: "$Medias.PostedOn" },
          },
        },
      },
      {
        $project: {
          Medias: "$Medias",
          index: { $indexOfArray: ["$Medias._id", new ObjectId(PostId)] },
          size: { $size: "$Medias" },
        },
      },
    ],
      ...
      if (!err) {
        if (results.length) {
          ...
          if (Action == "ShiftRight") {
            ...
            var post2_obj = req.body.postObj2.Medias
              ? req.body.postObj2.Medias
              : null;
            if (!post2_obj) {
              var response = {
                status: 501,
...
var createEmail = function (req, res) {
  ...
  if (inputObj.page_id && inputObj.AfterDays) {
    ...
    var setObj = {
      $push: {
        EmailEngineDataSets: {
          ...
          TextAboveVisual: inputObj.TextAboveVisual
            ? inputObj.TextAboveVisual
            : "",
          TextBelowVisual: inputObj.TextBelowVisual
            ? inputObj.TextBelowVisual
            : "",
    ...
    var options = { multi: false };
    Page.updateOne(conditions, setObj, options, function (err, result) {
      if (!err) {
        //this will update the IsDasheditpage case
        var conditions2 = {
...
var editEmail = function (req, res) {
  ...
  if (inputObj.page_id) {
    ...
    var setObj = {
      $set: {
        EmailEngineDataSets: inputObj.EmailEngineDataSets
...
var searchMedia = async function (req, res) {
  ...
  var selectedKeywords = req.body.generatedKeywords
  ...
  var mediaCount = await Media.countDocuments(conditions);

  mediaCount = mediaCount ? mediaCount : 0;

  Media.aggregate(aggregateStages)
    .allowDiskUse(true)
    .exec(function (err, results) {
      if (err) {
        ...
        req.body.subsetByRankObj2 = req.body.subsetByRankObj2
          ? req.body.subsetByRankObj2
          : {};
        if (selectedKeywords.length) {
          return res.json({
            status: "success",
            ...
            SearchedKeywordByRank: req.body.subsetByRankObj2,
          });
        } else {
          return res.json({
            status: "success",
...
// Old function removed - using getMediaFromSet instead
...
var getMediaFromSet = async function (req, callback) {
  ...
  if (!subsetByRank || !Array.isArray(subsetByRank) || subsetByRank.length === 0) {
    console.log("❌ ERROR: subsetByRank is empty or invalid - cannot proceed with aggregation");
    console.log("❌ Returning empty results to prevent infinite loop");
    clearInterval(timer);
  ...
  reqObj.MediaSelectionCriteria1 = !reqObj.MediaSelectionCriteria1
    ? {}
    : reqObj.MediaSelectionCriteria1;
  reqObj.MediaSelectionCriteria = Object.values(reqObj.MediaSelectionCriteria1)
    .length
    ? reqObj.MediaSelectionCriteria1
    : null;
  ...
  let mediaSelectionCriteria =
    typeof reqObj.MediaSelectionCriteria === "object"
      ? reqObj.MediaSelectionCriteria
      : null;
  const mediaSelectionCriteriaKeyArr = Object.keys(
  ...
  if (selectedKeywords.length) {
    ...
    // Will extract ObjectIds from totalSets after it's defined
    conditions["GroupTags.GroupTagID"] = { $in: selectedKeywords };
    ...
    /*if(SecondaryKeywords.length) {
			...
			conditions["$and"].push({
				$or : [
					{ "GroupTags.GroupTagTitle" : { $in : selectedKeywords } }
				]
			});
		}*/
    aggregateStages.push({ $match: conditions });
    ...
    //code to make individual keyword in a set (MATCH OLD CODE - always convert to individual)
    ...
    var oneGT_subsetByRankObj2 = [];
      for (var rankKey in subsetByRankObj2) {
        if (subsetByRankObj2.hasOwnProperty(rankKey) && Array.isArray(subsetByRankObj2[rankKey])) {
          for (let j = 0; j < subsetByRankObj2[rankKey].length; j++) {
            oneGT_subsetByRankObj2.push([subsetByRankObj2[rankKey][j]]);
          }
        }
    }
    ...
    // Extract ObjectIds from "title__id" strings for aggregation matching
    totalSets = totalSets.map(set => 
      set.map(item => {
        if (typeof item === 'string' && item.includes('__')) {
          let parts = item.split('__');
          return parts.length >= 2 ? parts[1] : item; // Extract ObjectId part
        }
        return item;
      })
    );
    ...
    for (let i = 0; i < mediaSelectionCriteriaKeyArr.length; i++) {
      ...
      mediaSelectionCriteria[property] =
        typeof mediaSelectionCriteria[property] === "string"
          ...
          : mediaSelectionCriteria[property];
      mediaSelectionCriteria[property] =
        Array.isArray(mediaSelectionCriteria[property]) ?
      ...
      if (Array.isArray(mediaSelectionCriteria[property])) {
        for (let br = 0; br < mediaSelectionCriteria[property].length; br++) {
          branches.push({
            case: {
              $eq: [`$${property}`, mediaSelectionCriteria[property][br]],
            },
            then: [`${mediaSelectionCriteria[property][br]}`],
          });
          keyValArr.push({
            $eq: ["$$type", mediaSelectionCriteria[property][br]],
          });
        }
      } else {
        branches.push({
          case: {
            $eq: [`$${property}`, `${mediaSelectionCriteria[property]}`],
          },
          then: [`${mediaSelectionCriteria[property]}`],
        });
        keyValArr.push({
          $eq: ["$$type", `${mediaSelectionCriteria[property]}`],
        });
      }
    ...
    let projectPipeline_2 = {
      ...
      value: {
        ...
        MediaSelectionCriteriaCount: {
          $size: { $ifNull: ["$value.MediaSelectionCriteriaArr", []] },
        },
      },
    };
  ...
  // Remove limit - let MongoDB handle it naturally
  ...
  try {
    ...
    console.log("❌ ERROR in getMediaFromSet aggregation:", err.message);
    returnObj = {
      "status":"error",
      "results":outputRecords,
      "mediaCount":mediaCount,
...
var getMediaFromSet2 = async function (req, callback) {
  ...
  var subsetByRank = reqObj.subsetByRank ? reqObj.subsetByRank : [];
  var subsetByRankObj2 = reqObj.subsetByRankObj2
  ...
  reqObj.MediaSelectionCriteria2 = !reqObj.MediaSelectionCriteria2
    ? {}
    : reqObj.MediaSelectionCriteria2;
  reqObj.MediaSelectionCriteria = Object.values(reqObj.MediaSelectionCriteria2)
    .length
    ? reqObj.MediaSelectionCriteria2
    : null;
  /*{
    ...
    for (let i = 0; i < mediaSelectionCriteriaKeyArr.length; i++) {
      ...
      mediaSelectionCriteria[property] =
        typeof mediaSelectionCriteria[property] === "string"
          ? mediaSelectionCriteria[property].split(",")
          : mediaSelectionCriteria[property];
      mediaSelectionCriteria[property] = Array.isArray(
        mediaSelectionCriteria[property]
      )
        ? mediaSelectionCriteria[property].map((obj) => {
            if (typeof obj === "string") {
      ...
      if (keyValArr.length) {
        branches.push({
          case: {
            $isArray: `$${property}` /*, "$in": [keyValArr[0]["$eq"][1], `$${property}`]*/,
          },
          then: {
            $filter: {
              input: `$${property}`,
...
var streamPost_withEmailSync = function (req, res) {
  ...
  if (results.length == 1) {
    ...
    for (var i = 0; i < EmailEngineDataSets.length; i++) {
      ...
      EmailEngineDataSets[i].TextBelowVisual = EmailEngineDataSets[i]
        ...
        ? EmailEngineDataSets[i].TextBelowVisual
        : "";
      EmailEngineDataSets[i].SoundFileUrl = EmailEngineDataSets[i].SoundFileUrl
        ? EmailEngineDataSets[i].SoundFileUrl
        : null;
      if (!EmailEngineDataSets[i].VisualUrls.length) {
        BlendImage1 =
        ...
        if (BlendImage1 && BlendImage2) {
          //assign blend mode
          BlendImage1.Lightness = BlendImage1.Lightness
            ? parseFloat(BlendImage1.Lightness)
            : null;
          BlendImage2.Lightness = BlendImage2.Lightness
            ? parseFloat(BlendImage2.Lightness)
            : null;
    ...
    //#week 1
    BlendImage1 =
      results[0].Medias[Math.floor(Math.random() * results[0].Medias.length)];
    BlendImage2 =
      results[0].Medias[Math.floor(Math.random() * results[0].Medias.length)];
    ...
    for (var i = 0; i < EmailEngineDataSets.length; i++) {
      ...
      if (!EmailEngineDataSets[i].VisualUrls.length) {
        ...
        BlendImage1 = results[0].Medias[i + 1]
          ? results[0].Medias[i + 1]
          : results[0].Medias[
              Math.floor(Math.random() * results[0].Medias.length)
            ];
        BlendImage2 = results[1].Medias[i + 1]
          ? results[1].Medias[i + 1]
          : results[1].Medias[
              Math.floor(Math.random() * results[1].Medias.length)
            ];
    ...
    BlendMode = "hard-light";
    //#week 1
    BlendImage1 = results[0].Medias[0]
      ? results[0].Medias[0]
      : results[0].Medias[Math.floor(Math.random() * results[0].Medias.length)];
    BlendImage2 = results[1].Medias[0]
      ? results[1].Medias[0]
      : results[1].Medias[Math.floor(Math.random() * results[1].Medias.length)];
  ...
  var dataRecord = {
    ...
    EmailTemplate: req.body.EmailTemplate
      ...
      : "PracticalThinker", //ImaginativeThinker, PracticalThinker
    Status: req.body.IsStreamPaused ? 0 : 1,
    EmailSubject: req.body.EmailSubject ? req.body.EmailSubject : null,
    IsOnetimeStream: req.body.IsOnetimeStream
      ? req.body.IsOnetimeStream
      : false,
    IsOnlyPostImage: req.body.IsOnlyPostImage
        ...
        EmailTemplate.find(condition, {}, function (err, results) {
          if (!err) {
            if (results.length) {
              ...
              var SharedByUserName = req.session.user.Name
                ? req.session.user.Name.split(" ")[0]
                : req.body.SyncedByName
                ? req.body.SyncedByName.split(" ")[0]
                : "";
              var PostURL =
                "https://www.scrpt.com/post_view?post=" + dataRecord.PostId;
...
var streamPost_withEmailSync_v2 = async function (req, res) {
  ...
  if (results.length == 1) {
    ...
    for (var i = 0; i < EmailEngineDataSets.length; i++) {
      ...
      EmailEngineDataSets[i].DateOfDelivery =
        __StreamTYPE == "Group"
          ...
          : getDateIncrementedBy_CreatedOn(NoOfDays, CreatedOn);
      EmailEngineDataSets[i].VisualUrls = EmailEngineDataSets[i].VisualUrls
        ? EmailEngineDataSets[i].VisualUrls
        : [];
      EmailEngineDataSets[i].TextAboveVisual = EmailEngineDataSets[i]
    ...
    for (var i = 0; i < EmailEngineDataSets.length; i++) {
      var NoOfDays = EmailEngineDataSets[i].AfterDays
        ...
        : 0;
      EmailEngineDataSets[i].Delivered = false;
      EmailEngineDataSets[i].DateOfDelivery = getDateIncrementedBy_CreatedOn(
        NoOfDays,
        CreatedOn
      );
      EmailEngineDataSets[i].VisualUrls = EmailEngineDataSets[i].VisualUrls
  ...
  var dataRecord = {
    ...
    IsOnlyPostImage: req.body.IsOnlyPostImage
      ? req.body.IsOnlyPostImage
      : false,
    IsPrivateQuestionPost: req.body.IsPrivateQuestionPost
      ? req.body.IsPrivateQuestionPost
      : false,
  };
  dataRecord.ReceiverEmails =
    typeof dataRecord.ReceiverEmails == "object"
  ...
  if (
    ...
    if (err) {
      ...
      req.body.instantEmail = req.body.instantEmail
        ? req.body.instantEmail
        : false;
      ...
      EmailTemplate.find(condition, {}, function (err, results) {
        if (!err) {
          if (results.length) {
            ...
            PostImage1 = PostImage1.replace(
              "/Media/img/300/",
              "/Media/img/600/"
            );
            PostImage2 = PostImage2.replace(
              "/Media/img/300/",
              "/Media/img/600/"
            );
  ...
  async_lib.parallel(
    {
      mediaFromFirstSet: function (callback) {
        req.body.subsetByRank = req.body.subsetByRank
          ? req.body.subsetByRank
          : [];
        getMediaFromSet(req, callback);
        //callback(null, 'abc\n');
      },
      mediaFromSecondSet: function (callback) {
        req.body.subsetByRank2 = req.body.subsetByRank2
          ? req.body.subsetByRank2
          : [];
    ...
    function (err, results) {
      ...
      var MediaSet1 = results.mediaFromFirstSet.results
        ? results.mediaFromFirstSet.results
        : [];
      var MediaSet2 = results.mediaFromSecondSet.results
...
var streamPage__WithSelectedBlendCase = async function (req, res) {
  ...
  for (var i = 0; i < EmailEngineDataSets.length; i++) {
    ...
    EmailEngineDataSets[i].SoundFileUrl = EmailEngineDataSets[i].SoundFileUrl
      ? EmailEngineDataSets[i].SoundFileUrl
      : null;
    EmailEngineDataSets[i].BlendMode = EmailEngineDataSets[i].BlendMode
      ? EmailEngineDataSets[i].BlendMode
      : "hard-light";
    EmailEngineDataSets[i].SelectedKeywords = EmailEngineDataSets[i]
      .SelectedKeywords
      ? EmailEngineDataSets[i].SelectedKeywords
      : [];
  }

  var PostImage1 =
    EmailEngineDataSets[0].VisualUrls.length == 2
      ? EmailEngineDataSets[0].VisualUrls[0]
      : PostImage;
  var PostImage2 =
    EmailEngineDataSets[0].VisualUrls.length == 2
      ? EmailEngineDataSets[0].VisualUrls[1]
      : PostImage;
  BlendMode = EmailEngineDataSets[0].BlendMode
    ? EmailEngineDataSets[0].BlendMode
    : "hard-light";
...
var getBlendImages = async function (req, res) {
  req.session = req.session ? req.session : {};
  req.session.user = req.session.user ? req.session.user : {};
  req.session.user.onlyunsplash = req.session.user.onlyunsplash
    ? req.session.user.onlyunsplash
    : false;
  req.session.user.onlyenablers = req.session.user.onlyenablers
    ? req.session.user.onlyenablers
    : false;

  var keywords = req.body.SurpriseSelectedWords
    ? req.body.SurpriseSelectedWords
    : [];
  ...
  async_lib.parallel(
    ...
    async function (err, results) {
      ...
      if (SavedStreamData) {
        alreadySelectedBlends = SavedStreamData.SelectedBlendImages
          ? SavedStreamData.SelectedBlendImages
          : [];
      }
      ...
      for (var i = 0; i < MediaSet1Length; i++) {
        ...
        rankObj["rank_" + record.value.Ranks] = rankObj[
          "rank_" + record.value.Ranks
        ]
          ? rankObj["rank_" + record.value.Ranks]
          : [];
        if (
          rankObj["rank_" + record.value.Ranks].length >= noOfMediaPerRankLimit
        ) {
          continue;
        }
        ...
        tempArr.push({
          ...
          DominantColors: record.value.DominantColors
            ? record.value.DominantColors
            : null,
          SelectedGtTitle: record.value.SelectedGtTitle
            ? record.value.SelectedGtTitle
            : "",
          SecondaryKeywords: record.value.SecondaryKeywords
            ? record.value.SecondaryKeywords
            : [],
          SecondaryKeywordsCount: record.value.SecondaryKeywordsCount
            ? record.value.SecondaryKeywordsCount
            : 0,
          SecondaryKeywordsMap: record.value.SecondaryKeywordsMap
            ? record.value.SecondaryKeywordsMap
            : [],
          MediaSelectionCriteriaCount: record.value.MediaSelectionCriteriaCount
            ? record.value.MediaSelectionCriteriaCount
            : 0,
          MediaSelectionCriteriaArr: record.value.MediaSelectionCriteriaArr
            ? record.value.MediaSelectionCriteriaArr
            : [],
      ...
      for (var i = 0; i < MediaSet2Length; i++) {
        ...
        rankObj2["rank_" + record.value.Ranks] = rankObj2[
          "rank_" + record.value.Ranks
        ]
          ? rankObj2["rank_" + record.value.Ranks]
          : [];
        if (
          rankObj2["rank_" + record.value.Ranks].length >= noOfMediaPerRankLimit
        ) {
          continue;
        }
      ...
      for (var loop = 0; loop < selectedLengthForLoop; loop++) {
        ...
        for (var loop2 = 0; loop2 < alreadySelectedBlends.length; loop2++) {
          ...
          if (
            blendImage1 == savedData.blendImage1 &&
            blendImage2 == savedData.blendImage2
          ) {
            finalMediaArr[0].Medias[loop].blendMode = savedData.blendMode
              ? savedData.blendMode
              : null;
            finalMediaArr[1].Medias[loop].blendMode = savedData.blendMode
              ? savedData.blendMode
              : null;
            ...
            finalMediaArr[0].Medias[loop].SelectedGtTitle =
              savedData.Keywords.length >= 2 ? savedData.Keywords[0] : null;
            finalMediaArr[1].Medias[loop].SelectedGtTitle =
              savedData.Keywords.length >= 2 ? savedData.Keywords[1] : null;
...
var updateLightnessScore = async function (req, res) {
  ...
  try {
    ...
    for (var i = 0; i < inputArr.length; i++) {
      ...
      var options = { multi: false };
      let result = await Media.updateOne(conditions, { $set: setObj });
      result = result || {};
    ...
    return res.json({
      status: "success",
      message: `${successCounter} records updated successfully.`,
    });
  } catch (err) {
    console.log(err);
...
var savePostStreamConfig = async function (req, res) {
  var dataRecord = {
    PageId: req.body.PageId ? req.body.PageId : null,
    PostId: req.body.PostId ? req.body.PostId : null,
    SelectedBlendImages: req.body.SelectedBlendImages
      ? req.body.SelectedBlendImages
      : [],
  };
  ...
  if (dataRecord.PageId && dataRecord.PostId) {
    ...
    try {
      const result = await PageStream.updateOne(
        conditions,
        { $set: dataRecord },
        { upsert: true }
      );
...
var getUserStreamsStats = async function (req, res) {
  ...
  var InvitedInResult = await StreamMembers.find(
    { Members: ObjectId(req.session.user._id) },
    { StreamId: 1 }
  );
  ...
  if (results.length) {
    ...
    for (var loop = 0; loop < results.length; loop++) {
      ...
      var OwnerDetails =
        typeof results[loop].OwnerId == "object" ? results[loop].OwnerId : {};
      var PurchasedByObj =
        typeof results[loop].PurchasedBy == "object"
          ? results[loop].PurchasedBy
          : {};
      ...
      results[loop].OwnerProfilePic = OwnerDetails.ProfilePic
        ? OwnerDetails.ProfilePic
        : "";
      results[loop].OwnerBirthdate = OwnerDetails.Birthdate
        ? OwnerDetails.Birthdate
        : "";
      results[loop].OwnerReferralCode = OwnerDetails.referralCode
        ? OwnerDetails.referralCode
        : "";

      results[loop].PurchasedByName = PurchasedByObj.Name
        ? PurchasedByObj.Name
        : "";
      results[loop].PurchasedBy = PurchasedByObj._id ? PurchasedByObj._id : "";
      results[loop].PurchasedByProfilePic = PurchasedByObj.ProfilePic
        ? PurchasedByObj.ProfilePic
        : "";

      results[loop].LaunchSettings.StreamType = results[loop].LaunchSettings
        .StreamType
        ? results[loop].LaunchSettings.StreamType
        : "";
      results[loop].IsSurpriseGift = results[loop].IsSurpriseGift
        ? results[loop].IsSurpriseGift
        : false;

      if (
        results[loop].LaunchSettings.StreamType == "Group" &&
        results[loop].OwnerId != String(req.session.user._id)
      ) {
        results[loop].Title =
          results[loop].OwnerName.split(" ")[0] + "'s " + results[loop].Title;
        if (results[loop].IsSurpriseGift) {
          if (
            String(results[loop].PurchasedBy) == String(req.session.user._id)
          ) {
            ...
            OwnerDetails.Birthdate = OwnerDetails.Birthdate
              ? OwnerDetails.Birthdate
              : null;
          ...
          if (!results[loop].LaunchSettings.Audience == "CELEBRITY") {
            results[loop].Title =
              results[loop].OwnerName.split(" ")[0] +
              "'s " +
              results[loop].Title;
          }
        }
      ...
      if (
        ...
        results[loop].OwnerId == String(req.session.user._id) &&
        !results[loop].IsSurpriseGift &&
        String(results[loop].PurchasedBy) != "" &&
        String(results[loop].PurchasedBy) != String(req.session.user._id)
      ) {
        results[loop].Title = results[loop].Title; // + " (Gifted by "+ results[loop].PurchasedByName.split(' ')[0]+")";
        results[loop].MetaData = results[loop].MetaData
          ? results[loop].MetaData
          : {};
        results[loop].MetaData.StickerTextOwner =
          "Gifted by " + results[loop].PurchasedByName.split(" ")[0];
      }
      ...
      if (
        ...
        results[loop].Title =
          results[loop].Title +
          " (Surprise from " +
          results[loop].PurchasedByName.split(" ")[0] +
          ")";
      ...
      if (CapsuleId && UserEmail) {
        ...
        StreamOpenedEmailCount = await StreamEmailTracker.find(
          conditions,
          {}
        ).count();
        StreamOpenedEmailCount = StreamOpenedEmailCount
          ? StreamOpenedEmailCount
          : 0;
        ...
        var result = await StreamConversation.findOne(
          StreamConversation_conditions,
          {}
        );
        result = result ? result : {};
        result.ConversationCount = result.ConversationCount
      ...
      if (
        results[loop].LaunchSettings.Audience == "CELEBRITY" &&
        results[loop].OwnerId == String(req.session.user._id)
      ) {
...
var addConversation = async function (req, res) {
  ...
  var updateResult = await StreamConversation.update(
    conditions,
    { $set: dataRecord },
    { upsert: true }
  );
...
var savePostStreamKeywords = function (req, res) {
  var PostId = req.body.PostId ? req.body.PostId : null;
  var SurpriseSelectedWords = req.body.SurpriseSelectedWords
    ? req.body.SurpriseSelectedWords
    : [];
...
var setmysession = function (req, res) {
  ...
  req.session.user =
    typeof req.session.user == "object" ? req.session.user : null;
  if (!req.session.user) {
    return res.send("Not Authenticated. Please login to change your settings.");
  }

  req.query.onlyunsplash = req.query.onlyunsplash
    ? req.query.onlyunsplash
    : null;
  req.query.onlyenablers = req.query.onlyenablers
    ? req.query.onlyenablers
    : null;

  if (req.query.onlyunsplash == null && req.query.onlyenablers == null) {
    return res.json({
      message:
        "Settings have been saved successfully. you can set your page stream now.",
      settings: {
        onlyunsplash: req.session.user.onlyunsplash,
        onlyenablers: req.session.user.onlyenablers,
      },
    });
  }
  ...
  return res.json({
    message:
      "Settings have been saved successfully. you can set your page stream now.",
    settings: { onlyunsplash: onlyunsplash, onlyenablers: onlyenablers },
  });
};

function generateBlendImage(
  blendImage1,
  blendImage2,
  blendOption,
  uploadDir,
  ...
  if (blendImage1 && blendImage2 && blendOption) {
    if (blendImage1.indexOf("unsplash.com") == -1) {
      ...
      blendImage1 =
        blendImage1.split("&")[0] +
        "&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=600&h=600&fit=crop&ixid=eyJhcHBfaWQiOjEyMDd9";

      downloadFileSrc1 =
        __dirname +
        "/../../media-assets/unsplash_store/" +
        "blendImage1_" +
    ...
    function nextStep_1() {
      if (blendImage2.indexOf("unsplash.com") == -1) {
        ...
        var commandToDownload =
          "curl -o " + downloadFileSrc2 + " " + blendImage2;
        commandToDownload = commandToDownload.replace("https://", "http://");
...
async function generateBlendImage_returnJson(
  blendImage1,
  blendImage2,
  blendOption,
  uploadDir,
  ...
  try {
    console.log("🔄 Generating blend image with Sharp...");

    if (!blendImage1 || !blendImage2 || !blendOption) {
      console.log("Missing required parameters for blend generation");
      return res.json({ code: 501, message: "Missing required parameters." });
    }

    // Process base64 image data directly
    let image1Buffer, image2Buffer;
    
    try {
      // Handle base64 data
      if (blendImage1.startsWith('data:image/')) {
        // Remove data URL prefix
        const base64Data1 = blendImage1.split(',')[1];
        image1Buffer = Buffer.from(base64Data1, 'base64');
                } else {
        // Direct base64
        image1Buffer = Buffer.from(blendImage1, 'base64');
      }
      
      if (blendImage2.startsWith('data:image/')) {
        // Remove data URL prefix
        const base64Data2 = blendImage2.split(',')[1];
        image2Buffer = Buffer.from(base64Data2, 'base64');
      } else {
        // Direct base64
        image2Buffer = Buffer.from(blendImage2, 'base64');
      }
      
      
    } catch (error) {
      console.log("❌ Failed to process base64 data:", error.message);
      return res.json({
        code: 501,
        message: "Failed to process base64 image data: " + error.message,
        receivedData1: blendImage1.substring(0, 100) + "...",
        receivedData2: blendImage2.substring(0, 100) + "..."
      });
    }


    // Process images with Sharp
    const image1 = sharp(image1Buffer).resize(600, 600, { fit: "cover" });
    const image2 = sharp(image2Buffer).resize(600, 600, { fit: "cover" });

    let blendedImage;

    // Apply blend mode based on blendOption
      switch (blendOption) {
      case "overlay":
        // Custom overlay that shows both images clearly - using screen blend with opacity
        const overlayImage = await image2.toBuffer();
        blendedImage = await image1.composite([
          { 
            input: overlayImage, 
            blend: "screen",
            opacity: 0.5  // 50% opacity to show both images
          },
        ]);
          break;
      case "overlay-light":
        // Lighter overlay - 30% opacity for subtle blend
        const lightOverlayImage = await image2.toBuffer();
        blendedImage = await image1.composite([
          { 
            input: lightOverlayImage, 
            blend: "screen",
            opacity: 0.3  // 30% opacity for subtle blend
          },
        ]);
          break;
      case "overlay-strong":
        // Stronger overlay - 70% opacity for more visible second image
        const strongOverlayImage = await image2.toBuffer();
        blendedImage = await image1.composite([
          { 
            input: strongOverlayImage, 
            blend: "screen",
            opacity: 0.7  // 70% opacity for stronger blend
          },
        ]);
          break;
        case "multiply":
        // Shows both images with darker result
        blendedImage = await image1.composite([
          { input: await image2.toBuffer(), blend: "multiply" },
        ]);
          break;
        case "screen":
        // Shows both images with lighter result
        blendedImage = await image1.composite([
          { input: await image2.toBuffer(), blend: "screen" },
        ]);
          break;
      case "soft-light":
        // Gentle blend showing both images
        blendedImage = await image1.composite([
          { input: await image2.toBuffer(), blend: "soft-light" },
        ]);
          break;
      case "hard-light":
        // Stronger blend
        blendedImage = await image1.composite([
          { input: await image2.toBuffer(), blend: "hard-light" },
        ]);
        break;
      case "difference":
        // Shows differences between images
        blendedImage = await image1.composite([
          { input: await image2.toBuffer(), blend: "difference" },
        ]);
        break;
      case "lighten":
        // Shows lighter parts of both images
        blendedImage = await image1.composite([
          { input: await image2.toBuffer(), blend: "lighten" },
        ]);
          break;
        case "darken":
        // Shows darker parts of both images
        blendedImage = await image1.composite([
          { input: await image2.toBuffer(), blend: "darken" },
        ]);
          break;
        default:
        // Default to custom overlay - shows both images clearly with 50% opacity
        const defaultOverlayImage = await image2.toBuffer();
        blendedImage = await image1.composite([
          { 
            input: defaultOverlayImage, 
            blend: "screen",
            opacity: 0.5  // 50% opacity to show both images
          },
        ]);
    ...
    const blendedBuffer = await blendedImage.png().toBuffer();


    // Upload to S3
    const s3Key = `scrptMedia/img/aspectfit/${file_name}`;
    ...
    if (uploadResult.success) {
      // Generate S3 URL manually since uploadBufferToS3 might not return URL
      const bucket = process.env.AWS_BUCKET_NAME || "scrpt";
      const region = process.env.AWS_REGION || "us-east-1";
      const s3Url = `https://${bucket}.s3.${region}.amazonaws.com/${s3Key}`;
      
      console.log("✅ Blend image uploaded to S3:", s3Url);

      // Return success response
      return res.json({
        code: 200,
        message: "Blend image generated and uploaded successfully",
        blendImageUrl: s3Url,
        s3Key: s3Key,
      });
        } else {
      console.log("❌ Failed to upload blend image to S3:", uploadResult.error);
      return res.json({
        code: 501,
        message: "Failed to upload blend image to S3: " + uploadResult.error,
      });
    }
  } catch (error) {
    console.log("❌ Error generating blend image:", error.message);
          return res.json({
      code: 501,
      message: "Error generating blend image: " + error.message,
    });
  }
}
  ...
  if (dataRecord.PageId && dataRecord.PostId) {
    ...
    for (var i = 0; i < totalRecords; i++) {
      try {
        ...
        var blendOption = obj.blendMode ? obj.blendMode : "overlay";
    ...
    return res.json({
      code: 200,
      message: `Total ${
        totalRecords - errCounter
      } blended images have been generated successfully.`,
    });
  } else {
    return res.json({
      code: "204",
      message:
        "It seems like you have not yet selected the blended images for this post.",
    });
  }
};
    ...
    if (blendImage1 && blendImage2 && blendOption) {
      ...
      if (hexcode) {
        ...
        if (fs.existsSync(uploadDir + "/" + file_name)) {
          ...
          return res.json({
            code: 200,
            message: "Blend Image was already generated.",
            status: "AlreadyGenerated",
          });
        } else {
          await generateBlendImage_returnJson(
            blendImage1,
            blendImage2,
            blendOption,
            uploadDir,
        ...
        return res.json({
          code: 501,
          message: "error in hex code generation.",
        });
      }
    } else {
      console.log("Error in input data.");
  ...
  try {
    ...
    if (blendImage1 && blendImage2 && blendOption) {
      ...
      if (hexcode) {
        ...
        // Check if blend already exists locally
        if (fs.existsSync(uploadDir + "/" + file_name)) {
          console.log(
            "🔄 Blend image already exists locally, uploading to S3..."
          );

          // Upload existing blend to S3
          try {
            const blendImageBuffer = fs.readFileSync(
              uploadDir + "/" + file_name
            );
            const s3Key = `scrptMedia/img/aspectfit/${file_name}`;
            ...
            if (uploadResult.success) {
              ...
              // Update Media document with S3 URL
              if (mediaId) {
                await Media.updateOne(
                  { _id: mediaId },
                  {
                    $set: {
                      ...
                      "Location.0.Size": "aspectfit",
                      "BlendSettings.S3Url": s3Url,
                      "BlendSettings.S3Key": s3Key,
              ...
              console.log("✅ Existing blend uploaded to S3 successfully");
              console.log("Blend S3 URL:", s3Url);

          return res.json({
            code: 200,
                message: "Blend Image uploaded to S3 successfully.",
                s3Url: s3Url,
                s3Key: s3Key,
          });
        } else {
            ...
            console.log(
              "❌ Error uploading existing blend to S3:",
              uploadError.message
            );
            return res.json({
              code: 501,
              message: "Error uploading blend to S3: " + uploadError.message,
            });
          }
        } else {
          // Generate new blend image and upload to S3
          console.log("🔄 Generating new blend image...");
          try {
          ...
          await generateBlendImage_returnJson(
            ...
            setTimeout(async () => {
              try {
                const blendImagePath = uploadDir + "/" + file_name;
                if (fs.existsSync(blendImagePath)) {
                  const blendImageBuffer = fs.readFileSync(blendImagePath);
                  const s3Key = `scrptMedia/img/aspectfit/${file_name}`;
                  ...
                  if (uploadResult.success) {
                    ...
                    // Clean up local file
                    fs.unlinkSync(blendImagePath);

                    console.log(
                      "✅ New blend image uploaded to S3 successfully"
                    );
                    console.log("Blend S3 URL:", s3Url);
                  } else {
                    console.log(
                      "❌ Failed to upload new blend to S3:",
                      uploadResult.error
                    );
                  }
                }
                ...
                console.log(
                  "❌ Error uploading new blend to S3:",
                  s3Error.message
                );
              }
            }, 2000); // Wait 2 seconds for blend generation to complete
          } catch (generationError) {
            return res.json({
              code: 501,
              message:
                "Error generating blend image: " + generationError.message,
            });
          }
        }
      } else {
...
var myStreamPosts = async function (req, res) {
  ...
  if (StreamType == "Group" && String(OwnerId) != String(loginUserId)) {
    ...
    AnswerFinishLogs_result = Array.isArray(AnswerFinishLogs_result)
      ? AnswerFinishLogs_result
      : [];
    ...
    if (AnswerFinishLogs_result.length) {
      if (IsOwnerPostsForMember) {
        if (IsPurchaseNeededForAllPosts) {
          ...
          var CapsuleData_MemberPurchase = await Capsule.find(
            CapsuleData_MemberPurchase_cond,
            { _id: 1 }
          );
          CapsuleData_MemberPurchase = Array.isArray(CapsuleData_MemberPurchase)
            ? CapsuleData_MemberPurchase
            : [];
    ...
    if (StreamType == "Group") {
      ...
      if (CapsuleData.StreamFlow == "Birthday") {
        ...
        CapsuleData.StreamFlow == "Topic" ||
        CapsuleData.StreamFlow == "Event"
      ) {
        if (!StreamLaunchDate) {
          return res.json({
            code: "200",
      ...
      if (
        String(req.session.user._id) == "5f4835e67a5d1b040c63237e" ||
        tmp_emails.indexOf(req.session.user.Email) >= 0
      ) {
        conditions = conditionsTempForNonGroupType;
        finalConditions = {};
        sortBy = { DateOfDelivery: -1 };
      ...
      if (
        String(req.session.user._id) == "5f4835e67a5d1b040c63237e" ||
        tmp_emails.indexOf(req.session.user.Email.toLowerCase()) >= 0
      ) {
        sortBy = { DateOfDelivery: 1 }; // Sets sorting to ascending (oldest first)
      }
    }
              ...
              if (blendImage1 && blendImage2 && blendOption) {
                if (blendImage1 != blendImage2) {
                  ...
                  if (hexcode) {
                    var file_name = hexcode + ".png";
                    var uploadDir =
                      __dirname + "/../../media-assets/streamposts";
  ...
  Page.aggregate(
    [
      ...
      },
      {
        $unwind: {
          path: "$AnswerPostsObj",
          includeArrayIndex: "AnswerPostsArrayIndex",
              ...
              $cond: [
                {
                  $and: [
                    ...
                    {
                      $eq: ["$AnswerPostsObj.PostedBy", new ObjectId(loginUserId)],
                    },
                  ],
                },
                //{ $gte: ["$AnswerPostsArrayIndex", 0] },
                  ...
                  VisualUrls: [
                    {
                      $switch: {
                        branches: [
                          {
                            case: {
                              $eq: ["$AnswerPostsObj.MediaType", "Video"],
                            },
                            then: {
                              $concat: [
                                "https://www.scrpt.com/assets/Media/video/600/",
                                "$AnswerPostsObj.thumbnail",
                              ],
                            },
                          },
                          {
                            case: {
                              $and: [
                                { $eq: ["$AnswerPostsObj.MediaType", "Link"] },
                                {
                                  $eq: [
                                    "$AnswerPostsObj.IsUnsplashImage",
                                    true,
                                  ],
                                },
                              ],
                            },
                            then: "$AnswerPostsObj.thumbnail",
                          },
                          {
                            case: {
                              $and: [
                          ...
                          },
                          {
                            case: {
                              $and: [
                                { $ne: ["$AnswerPostsObj.MediaType", "Video"] },
                                { $ne: ["$AnswerPostsObj.MediaType", "Link"] },
                              ],
                            },
                            then: {
                              $concat: [
...
var myStreamPosts__GroupStream_MemberCase_prefill = async function (req, res) {
  console.log(
    "------------- myStreamPosts__GroupStream_MemberCase_prefill ---------- "
  );
  var CapsuleId = req.body.CapsuleId ? req.body.CapsuleId : null;
  var loginUserId = req.session.user._id;
  var OwnerId = req.body.OwnerId ? req.body.OwnerId : null;
...
var userStreamsPostsWithActivities_GroupCase = async function (req, res) {
  ...
  SelectedTeam.MemberEmails = SelectedTeam.MemberEmails
    ? SelectedTeam.MemberEmails
    : [];
  SelectedTeam.StreamIds = StreamIds ? StreamIds : [];
  SelectedTeam.MemberEmails.push({
    Email: req.session.user.Email,
    StreamIds: SelectedTeam.StreamIds,
  });

  for (var i = 0; i < SelectedTeam.MemberEmails.length; i++) {
    if (SelectedTeam.MemberEmails[i].Email) {
      let sIds = SelectedTeam.MemberEmails[i].StreamIds
        ? SelectedTeam.MemberEmails[i].StreamIds
        : [];
      for (var j = 0; j < sIds.length; j++) {
        sIds[j] = ObjectId(sIds[j]);
      }
      UserEmailsAndStreamsMap[SelectedTeam.MemberEmails[i].Email] = sIds
        ? sIds
        : [];
    }
  }
  ...
  var UserArr = await User.find(
    { Email: { $in: UserEmails }, IsDeleted: 0, Status: 1 },
    { _id: 1, Email: 1 }
  );
  ...
  for (var i = 0; i < UserArr.length; i++) {
    ...
    if (UserEmailsAndStreamsMap[userObj.Email]) {
      UserEmailsAndStreamsMap[userObj.Email] = UserEmailsAndStreamsMap[
        userObj.Email
      ]
        ? UserEmailsAndStreamsMap[userObj.Email]
        : [];
      ...
      if (!err) {
        ...
        try {
          ...
          for (let loop = 0; loop < syncedPostsResults.length; loop++) {
            ...
            dataRecord1.PageData = Array.isArray(dataRecord1.PageData)
              ? dataRecord1.PageData[0]
              : {};
            dataRecord1.PageData.OriginatedFrom = dataRecord1.PageData
              .OriginatedFrom
              ? dataRecord1.PageData.OriginatedFrom
              : null;
            dataRecord1.PageData.Medias = dataRecord1.PageData.Medias
              ? dataRecord1.PageData.Medias
              : [];

            //there might be cases where the actual post or page has been deleted - In this case the post will no longer be visible in the social page

            dataRecord1.SocialPageId = dataRecord1.PageData.OriginatedFrom
              ? dataRecord1.PageData.OriginatedFrom
              : null;

            for (var i = 0; i < dataRecord1.PageData.Medias.length; i++) {
              if (
                String(dataRecord1.PostId) ==
                  String(dataRecord1.PageData.Medias[i]._id) &&
                dataRecord1.PageData.Medias[i].OriginatedFrom
              ) {
                dataRecord1.SocialPostId =
                  dataRecord1.PageData.Medias[i].OriginatedFrom;
                break;
              }
            }
          ...
          var SocialPostsWithCommentsArr = await StreamComments.find(conditions)
            .sort({ CreatedOn: -1 })
            .populate("UserId", "_id Name Email ProfilePic")
            .lean();
          SocialPostsWithCommentsArr = Array.isArray(SocialPostsWithCommentsArr)
          ...
          for (let loop = 0; loop < syncedPostsResults.length; loop++) {
            ...
            dataRecord.PageData =
              typeof dataRecord.PageData == "object" ? dataRecord.PageData : {};
            dataRecord.PageData.OriginatedFrom = dataRecord.PageData
              .OriginatedFrom
              ? dataRecord.PageData.OriginatedFrom
              : null;
          ...
          for (var i = 0; i < streamPosts.length; i++) {
            streamPosts[i].ActivityText = streamPosts[i].ActivityText
              ...
              : null;
            streamPosts[i].hexcode_blendedImage = streamPosts[i]
...
var getPrivateComments = async function (req, res) {
    ...
    },
    {
      $match: {
        $or: [
          ...
          { "ParentData.UserId": new ObjectId(loginUserId) },
        ],
      },
    },
    {
  ...
  for (var i = 0; i < comments.length; i++) {
    comments[i].UserId = Array.isArray(comments[i].UserId)
      ? comments[i].UserId.length > 0
        ? {
            _id: comments[i].UserId[0]._id,
            Name: comments[i].UserId[0].Name,
            Email: comments[i].UserId[0].Email,
            ProfilePic: comments[i].UserId[0].ProfilePic,
          }
        : {}
      : {};
...
var newPostsReplicaFromExistingStream = async function (req, res) {
  try {
    var existingStreamPageArr = await Page.find(
      { _id: ObjectId("6097cbeb3889f72df4c8b69e") },
      { Medias: 1 }
    ).lean();
    existingStreamPageArr = Array.isArray(existingStreamPageArr)
      ? existingStreamPageArr
      : [];
    if (!existingStreamPageArr.length) {
      return res.json({ code: 404, message: "Stream does not exist." });
    }
    var streamPosts = existingStreamPageArr[0].Medias
      ? existingStreamPageArr[0].Medias
      : [];
    //console.log("streamPosts - ", streamPosts);
    if (!streamPosts.length) {
      return res.json({
        code: 200,
        message: "There is no posts in existing stream.",
      });
    }

    var newStreamPageArr = await Page.find(
      { _id: ObjectId("6126746b4b631420cc5a9d36") },
      { _id: 1, CreaterId: 1, OwnerId: 1 }
    ).lean();
    ...
    if (!newStreamPageArr.length) {
      return res.json({
        code: 404,
        message: "We couldn't find record based on your new stream id.",
      });
    }
    ...
    for (var i = 0; i < streamPosts.length; i++) {
      ...
      var PageStreamDataArr = await PageStream.find({
        PageId: existingPageId,
        PostId: existingPostId,
      }).lean();
      PageStreamDataArr = Array.isArray(PageStreamDataArr)
        ? PageStreamDataArr
        : [];
    ...
    var results = await Page.updateOne(
      { _id: newStreamPageArr[0]._id },
      { $set: { Medias: newStreamPosts } }
    );
...
var markAsAdPost = async function (req, res) {
  console.log("=== markAsAdPost START ===");
  console.log("Request body:", JSON.stringify(req.body));
  console.log("PostId from request:", req.body.PostId);
  
  var PostId = req.body.PostId ? req.body.PostId : null;
  console.log("PostId after processing:", PostId);

  if (!PostId) {
    console.log("❌ No PostId provided - returning error");
    return res.json({
      status: 400,
  ...
  console.log("✅ PostId is valid, proceeding with database operations");
  ...
  console.log("📋 First query conditions:", JSON.stringify(conditions));
  ...
  console.log("📝 Set object:", JSON.stringify(setObj));

  var options = { multi: false };
  console.log("⚙️ Options:", JSON.stringify(options));
  
  // First, let's check if the media document exists
  console.log("🔍 Checking if media document exists with PostId...");
  
  try {
    const mediaDoc = await Media.findById(new ObjectId(PostId));
    
    if (!mediaDoc) {
      console.log("❌ No media document found with PostId:", PostId);
      return res.json({
        status: 404,
        message: "No media document found with the provided PostId"
      });
    }
    
    console.log("✅ Media document found:", mediaDoc._id);
    console.log("📄 Media title:", mediaDoc.Title);
    
    // Now update the media document directly
    console.log("🔄 Updating media document...");
    
    const updateData = {
      PostType: "AdPost"
    };
    
    const result = await Media.updateOne({ _id: new ObjectId(PostId) }, { $set: updateData });
    console.log("📊 Update result:", result);
    
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      console.log("✅ Media update successful - sending success response");
            var response = {
              status: 200,
        message: "Post marked as Ad Post successfully.",
        result: result
            };
      console.log("📤 Sending response:", JSON.stringify(response));
            return res.json(response);
          } else {
      console.log("❌ Media update failed - no documents modified");
            var response = {
              status: 501,
        message: "Media update failed - no documents modified"
            };
  ...
  console.log("=== markAsAdPost END ===");
};

var markAsBroadcastPost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    }
    ...
    // Update the media document directly
    const updateData = {
      PostType: "BroadcastPost"
    };
    
    const result = await Media.updateOne({ _id: new ObjectId(PostId) }, { $set: updateData });
    
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as Broadcast Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsBroadcastPost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

var markAsKeyPost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
  var KeyPostType = req.body.KeyPostType ? req.body.KeyPostType : "Comment";
    ...
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as Key Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsKeyPost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

var markAsGeneralPost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    ...
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as General Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsGeneralPost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

var markAsQuestionPost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    ...
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as Question Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsQuestionPost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

var markAsInfoPost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    ...
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as Info Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsInfoPost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

var markAsInfoPostOwner = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    ...
    // Update the media document directly
    const updateData = {
      PostType: "InfoPostOwner"
    };
    
    const result = await Media.updateOne({ _id: new ObjectId(PostId) }, { $set: updateData });
    
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as Info Post Owner successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsInfoPostOwner error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

//this is a sub case under InfoPostOwner type post
var markAsIsPreLaunchPost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    ...
    // Update the media document directly
    const updateData = {
      IsPreLaunchPost: true
    };
    
    const result = await Media.updateOne({ _id: new ObjectId(PostId) }, { $set: updateData });
    
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as Pre-Launch Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsIsPreLaunchPost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

var markAsNotPreLaunchPost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    ...
    // Update the media document directly
    const updateData = {
      IsPreLaunchPost: false
    };
    
    const result = await Media.updateOne({ _id: new ObjectId(PostId) }, { $set: updateData });
    
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as Not Pre-Launch Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsNotPreLaunchPost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

var markAsPrivateQuestionPost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    ...
    // Update the media document directly
    const updateData = {
      IsPrivateQuestionPost: true
    };
    
    const result = await Media.updateOne({ _id: new ObjectId(PostId) }, { $set: updateData });
    
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as Private Question Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsPrivateQuestionPost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

var markAsPublicQuestionPost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    ...
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as Public Question Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsPublicQuestionPost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

var markAsPost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    ...
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as Regular Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsPost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

var markAsOneTimePost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    ...
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as One-Time Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsOneTimePost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};

var markAsRepeatPost = async function (req, res) {
  try {
  var PostId = req.body.PostId ? req.body.PostId : null;
    if (!PostId) {
      return res.json({ status: 400, message: "PostId is required" });
    ...
    if (result.modifiedCount > 0 || result.matchedCount > 0) {
      return res.json({ status: 200, message: "Post marked as Repeat Post successfully." });
          } else {
      return res.json({ status: 501, message: "Media update failed - no documents modified" });
    }
  } catch (error) {
    console.log("markAsRepeatPost error:", error);
    return res.json({ status: 500, message: "Database operation failed", error: error.message });
  }
};
...
var getUserStats = async function (req, res) {
  ...
  var searchByTagName = req.body.searchByTagName
    ? req.body.searchByTagName
    : null;
  if (searchByTagName) {
  ...
  var KeyActionsArr = await SyncedPost.aggregate([
    ...
    { $match: { "StreamCommentsData.UserId": new ObjectId(loginUserId) } },
  ...
  return res.json({
    ...
    result: {
      ...
      CommentsCount: CommentsCount ? CommentsCount : 0,
      SharesWithFriendsCount: SharesWithFriendsCount
        ? SharesWithFriendsCount
        : 0,
    },
  });
};
...
async function sendMembersInvitationEmail(users, OwnerDetails, StreamId) {
  ...
  var IsOwnerPostsForMember = resultsStrm[0].IsOwnerPostsForMember
    ? resultsStrm[0].IsOwnerPostsForMember
    : false;
  var IsPurchaseNeededForAllPosts = resultsStrm[0].IsPurchaseNeededForAllPosts
    ? resultsStrm[0].IsPurchaseNeededForAllPosts
    : false;

  var MemberSharingStatements =
    "<br><br>The purpose of this experience is to provide {OwnerName} a unique opportunity for self-discovery.<br><br>Some of your answers will be transformed into a new series of visual posts. The series will create a private media journey for {OwnerName}. There is nothing like this on the planet!";

  if (IsOwnerPostsForMember) {
    MemberSharingStatements =
      "<br><br>Some of the answers will be transformed into a series of visual posts.<br><br>All members of the group will get to see each other’s answers and comment on them. Comments will also be visible to everyone in the group.<br><br>You will all get to open your posts on {LaunchDate}!";
    ...
    if (IsPurchaseNeededForAllPosts) {
      ...
      if (
        resultsStrm[0].LaunchSettings.StreamType == "Group" &&
        resultsStrm[0].StreamFlow === "Birthday"
      ) {
        MemberSharingStatements =
      ...
      if (
        ...
        MemberSharingStatements =
          "<br><br>Your text answers will be transformed into visual posts and delivered every few days, for up to a year. You and {OwnerName} will get to see your posts on {LaunchDate}! <br><br>you will see posts from other invited friends too, get to interact with them and award points to your favorite posts!";
      }
    }
  }
  ...
  var StreamFlow = resultsStrm[0].StreamFlow
    ? resultsStrm[0].StreamFlow
    : "Birthday";
  ...
  for (var i = 0; i < users.length; i++) {
    ...
    if (resultsStrm[0].IsSurpriseGift) {
      SurpriseGiftStatement = SurpriseGiftStatement.replace(
        /{OwnerName}/g,
        OwnerDetails.Name.split(" ")[0]
      );
      newHtml = newHtml.replace(
        /{SurpriseGiftStatement}/g,
        SurpriseGiftStatement
      );
    } else {
...
async function createMembersUserAccount(newUsers, OwnerDetails, StreamId) {
  ...
  var MemberSharingStatements =
    "<br><br>The purpose of this experience is to provide {OwnerName} a unique opportunity for self-discovery.<br><br>Your text answers will be transformed into a new series of visual posts. The series will create a private media journey for {OwnerName}. There is nothing like this on the planet!";

  if (IsOwnerPostsForMember) {
    MemberSharingStatements =
      "<br><br>Your text answers will be transformed into a series of visual posts.<br><br>All members of the group will get to see each other’s answers and comment on them. Comments will also be visible to everyone in the group.<br><br>You will all get to open your posts on {LaunchDate}!";
...
var stream__addMembers = async function (req, res) {
  ...
  alreadyExistsUsers = Array.isArray(alreadyExistsUsers)
    ? alreadyExistsUsers
    : [];
  var UserEmailIdMap = {};
...
var stream__publicaddMembers = async function (req, res) {
  ...
  if (strm_result == null) {
    ...
    if (!isAlreadyExists) {
      ...
      var result = await StreamMembers.update(conditions, {
        $set: strm_result,
      });
      result = typeof result == "object" ? result : null;
...
var addKeywordAndCallAddBlendImagesApi_INTERNAL_API_BackupAPI_V2 =
  async function (req, res) {
    ...
    if (
      PageId &&
      inputObj.PostId &&
      inputObj.PostType == "AnswerPost" &&
      inputObj.MediaType == "Notes" &&
      req.body.PostText
    ) {
      ...
      if (keywords.length == 2) {
        ...
        var options = { multi: false };
        await Page.updateOne(conditions, setObj, options);
        await Page.updateOne(conditions2, { $set: setObj }, options);
        ...
        console.log(
          "Calling addBlendImages_INTERNAL_API --------------------------------- ",
          reqObj
        );
        var request_url =
          "https://www.scrpt.com/journal/addBlendImages_INTERNAL_API";
        axios.post(request_url, reqObj).then((response) => {
          response.data = response.data ? response.data : {};
          response.data.code = response.data.code ? response.data.code : null;
          console.log(
            "------------ AXIOS (ONETIME - /journal/addBlendImages_INTERNAL_API)---- SelectedBlendImages with Post has been mapped successfully using api call ---------------",
            response.data.code
          );
        });
        ...
        return res.json({
          code: 200,
          message:
            "Keywords mapped with post and called api to map SelectedBlendImages.",
        });
      } else {
        return res.json({
          code: 200,
...
var addKeywordAndCallAddBlendImagesApi_INTERNAL_API = async function (
  req,
  res
) {
  var inputObj = {
  ...
  if (
    ...
    var keywordsObj = await fetchKeywordsFromText_PrimarySecondary(
      inputObj.PostText
    );
    ...
    if (keywords.length == 2) {
      ...
      var reqObj = {
        ...
        SurpriseSelectedWords: keywords ? keywords : [],
        SecondaryKeywords:
          (await __getKeywordIdsByNames(secondaryKeywords)) || [],
  ...
  async_lib.parallel(
    ...
    async function (err, results) {
      ...
      if (MediaSet1.length === 0 && MediaSet2.length === 0) {
        ...
        return res.json({
          code: "204",
          message: "No media found for the given keywords",
          results: []
        });
      }
      ...
      for (var loop = 0; loop < selectedLengthForLoop; loop++) {
        ...
        var obj = {
          ...
          SecondaryKeywordsCount_1: set1[loop].SecondaryKeywordsCount
            ...
            : 0,
          SecondaryKeywordsMap_1: set1[loop].SecondaryKeywordsMap
            ? set1[loop].SecondaryKeywordsMap
            : [],
          MediaSelectionCriteriaCount_1: set1[loop].MediaSelectionCriteriaCount
            ? set1[loop].MediaSelectionCriteriaCount
            : 0,
          MediaSelectionCriteriaArr_1: set1[loop].MediaSelectionCriteriaArr
            ? set1[loop].MediaSelectionCriteriaArr
            : [],
              ...
              await Media.updateOne(
                { _id: PostId },
                {
                  $set: {
                    ...
                    thumbnail: firstBlend.blendImage1 || "",
...
var addAnswerFinishStatus = async function (req, res) {
  ...
  if (questionPostIds.length) {
    ...
    givenAnswerPostsArr = Array.isArray(givenAnswerPostsArr)
      ? givenAnswerPostsArr
      : [];
  ...
  AnswerFinishLogsObj.StreamId = new ObjectId(StreamId);
  AnswerFinishLogsObj.MemberId = new ObjectId(MemberId);
...
var getAnswerStageStatus = async function (req, res) {
  ...
  return res.json({
    code: 200,
    IsAnswerStageFinished: result.length ? true : false,
  });
};
...
var createNewUserAccount_INTERNAL_API = async function (req, res) {
  var newUserId = null;
  var userData = null;
  var body = req.body.newUser ? req.body.newUser : req.body;
  ...
  body.Password = body.Password ? body.Password : getRandomPassword(12);

  if (!body.Email || !body.Name || !body.NickName || !body.Password) {
    return res.json({ code: 400, newUserId: newUserId, userData: userData, message: "Wrong input." });
  }

  // Check for existing user by email
  var existingUserByEmail = await User.find({
    Email: { $regex: new RegExp("^" + body.Email + "$", "i") },
    IsDeleted: false,
  });
  existingUserByEmail = Array.isArray(existingUserByEmail) ? existingUserByEmail : [];
  ...
  // Check for existing user by username/nickname
  var existingUserByNickname = await User.find({
    NickName: { $regex: new RegExp("^" + body.NickName + "$", "i") },
    IsDeleted: false,
  });
  existingUserByNickname = Array.isArray(existingUserByNickname) ? existingUserByNickname : [];
  ...
  try {
    ...
    userData = {
      ...
      Gender: numAffected.Gender,
      EmailConfirmationStatus: numAffected.EmailConfirmationStatus,
      CreatedOn: numAffected.CreatedOn,
      ModifiedOn: numAffected.ModifiedOn
    };
...
var sendUserSurpriseGiftNotification_INRENAL_API = async function (req, res) {
  ...
  var PurchasedByObj = await User.findOne(
    { _id: new ObjectId(PurchasedBy), IsDeleted: 0 },
    { Name: 1, Email: 1, Birthdate: 1 }
  );
  ...
  newHtml = results[0].description.replace(
    /{RecipientName}/g,
    OwnerDetails.Name
  );
  ...
  newHtml = newHtml.replace(
    /{OwnerBirthday}/g,
    OwnerBirthdate.getDate() + " " + Months[OwnerBirthdate.getMonth()]
  );
  newHtml = newHtml.replace(/{StreamPageUrl}/g, userStreamPageUrl);
  ...
  transporter.sendMail(mailOptions, function (error, info) {
    ...
    console.log(
      "sendUserSurpriseGiftNotification_INRENAL_API---------Message sent: " +
        info.response
    );
...
var sendPreLaunchPosts_INTERNAL_API = async function (req, res) {
  ...
  if (StreamData.StreamFlow == "Birthday" && !StreamData.LaunchDate) {
    return res.json({
      code: 501,
      message: "This Birthday Stream LaunchDate is not set.",
    });
  }
  ...
  console.log(
    "-------- sendPreLaunchPosts_INTERNAL_API posts --------",
    posts.length
  );
  for (var i = 0; i < posts.length; i++) {
    //send pre lauch post here
    var PostImage =
      posts[i].MediaType == "Link"
        ? posts[i].thumbnail
        : posts[i].MediaURL
        ? posts[i].MediaURL
        : "";
    PostImage =
      PostImage.indexOf("unsplash.com") == -1
        ? "https://www.scrpt.com/assets/Media/img/300/" + PostImage
        : PostImage;
    var PostStatement = posts[i].PostStatement
      ? posts[i].PostStatement
      : posts[i].Content;
    ...
    newHtml = newHtml.replace(
      /{LaunchDate}/g,
      Utilities.getFormattedDate(StreamData.LaunchDate)
    );
    ...
    transporter.sendMail(mailOptions, function (error, info) {
      ...
      console.log(
        "sendPreLaunchPosts_INTERNAL_API---------Message sent: " + info.response
      );
...
async function __getMyStreamAwardDetails(StreamId, loginUserId) {
  var conditions = {
    ...
    AwardedBy: new ObjectId(loginUserId),
  };
...
async function __getMyStreamAwardDetailsAllStreams(StreamIds, loginUserId) {
  var conditions = {
    ...
    AwardedBy: { $ne: new ObjectId(loginUserId) },
    PostOwnerId: new ObjectId(loginUserId),
  };
...
async function __getMyStreamAwardCountPerPost(StreamId, loginUserId) {
  ...
  for (var i = 0; i < result.length; i++) {
    ...
    groupedResultsMap[
      resultObj._id.Award +
        "_" +
        resultObj._id.PostId +
        "_" +
        resultObj._id.hexcode_blendedImage
    ] = {
      count: resultObj.count ? resultObj.count : 0,
      IsAwardedByMe: false,
    };
    resultObj.awardedByUsers = resultObj.awardedByUsers
      ? resultObj.awardedByUsers
      : [];
    resultObj.awardedByUsers = resultObj.awardedByUsers.map((obj) =>
      String(obj)
    );
...
var getMyStreamAwardDetails = async function (req, res) {
  ...
  var AllStreamIds = StreamIds.map((streamId) => new ObjectId(streamId));
  var StreamId = req.body.StreamId ? req.body.StreamId : null;
  var loginUserId = req.session.user._id;
  ...
  var StreamAwardCountPerPost = await __getMyStreamAwardCountPerPost(
    StreamId,
    loginUserId
  );
  var AllStreamAwardLogs = await __getMyStreamAwardDetailsAllStreams(
    AllStreamIds,
    loginUserId
  );
  ...
  return res.json({
    code: 200,
    result: awardObj,
    StreamAwardCountPerPost: StreamAwardCountPerPost,
    AllStreamAwardLogs: AllStreamAwardLogs,
  });
};
...
var markAward = async function (req, res) {
  ...
  var StreamId = req.body.StreamId ? req.body.StreamId : null;
  var PostId = req.body.PostId ? req.body.PostId : null;
  var hexcode_blendedImage = req.body.hexcode_blendedImage
    ? req.body.hexcode_blendedImage
    : null;
  ...
  var conditions = {
    ...
    hexcode_blendedImage: hexcode_blendedImage,
    StreamOwnerId: new ObjectId(StreamOwnerId),
    PostOwnerId: new ObjectId(PostOwnerId),
  };
  ...
  if (IsAlreadyExists) {
    return res.json({
      code: "404",
      message: "You have already awarded this post.",
    });
  }
...
var unsubscribeEmails = async function (req, res) {
  ...
  var PostActionsNotification =
    req.body.PostActionsNotification === false ? false : true;
  var CommentLikesNotification =
    req.body.CommentLikesNotification === false ? false : true;
  ...
  if (
    location == "Marketing" ||
    location === "PostActionsNotification" ||
    location === "CommentLikesNotification"
  ) {
    var conditions = {
      _id: UserData._id,
      IsDeleted: false,
...
var addKeywordsToGSheet = async function (req, res) {
  ...
  if (PageData) {
    ...
    for (var i = 0; i < PageData.Medias.length; i++) {
      var PostObj = PageData.Medias[i];
      var SurpriseSelectedWords =
        typeof PostObj.SurpriseSelectedWords === "string"
          ? PostObj.SurpriseSelectedWords.split(",")
          : [];
      var SecondaryKeywords =
        typeof PostObj.SecondaryKeywords === "string"
          ? PostObj.SecondaryKeywords.split(",")
          : [];
      ...
      if (uniqueKeywords.length) {
        try {
          for (var mp = 0; mp < uniqueKeywords.length; mp++) {
            var obj =
              typeof uniqueKeywords[mp] === "string"
                ? uniqueKeywords[mp].toLowerCase().trim()
                : "";
            if (obj) {
...
var updateAutoInviteCount = async function (req, res) {
  ...
  await Capsule.update(
    { _id: StreamId },
    { $set: { AutoInvitePopupCount: autoInvitePopupCount } }
  );
...
var updateAutoCoverPageCount = async function (req, res) {
  ...
  await Capsule.update(
    { _id: StreamId },
    { $set: { AutoCoverPageCount: autoCoverPageCount } }
  );
  return res.json({ code: 200, message: "Members added successfully." });
};
...
var getAutoPlayerSeenLog = async function (req, res) {
  ...
  if (UserId) {
    ...
    var uData = await User.findOne(conditions, {
      AutoPlayerSeenStreams: true,
      PostActionAnnouncementSeenStreams: true,
    });
    var AutoPlayerSeenStreams = uData.AutoPlayerSeenStreams || [];
    var PostActionAnnouncementSeenStreams =
      uData.PostActionAnnouncementSeenStreams || [];
    return res.json({
      code: 200,
      message: "Done.",
      AutoPlayerSeenStreams: AutoPlayerSeenStreams,
      PostActionAnnouncementSeenStreams: PostActionAnnouncementSeenStreams,
    });
  } else {
    return res.json({
      code: 500,
      message: "Wrong input.",
      AutoPlayerSeenStreams: [],
      PostActionAnnouncementSeenStreams: [],
    });
  }
};
...
var updateAutoPlayerSeenLog = async function (req, res) {
  ...
  if (UserId && StreamId) {
    ...
    var setObj = {
      $addToSet: { AutoPlayerSeenStreams: new ObjectId(StreamId) },
    };
    var options = {
      multi: false,
    };
...
var updatePostActionAnnouncementSeenLog = async function (req, res) {
  ...
  if (UserId && StreamId) {
    ...
    var setObj = {
      $addToSet: { PostActionAnnouncementSeenStreams: new ObjectId(StreamId) },
    };
    var options = {
      multi: false,
    };
...
var getQuestAudioStats = async function (req, res) {
  ...
  if (questIds.length) {
    for (var i = 0; i < questIds.length; i++) {
      var file =
        __dirname + "/../../media-assets/postaudios/" + questIds[i] + ".mp3";
...
async function __getPostHexCodeOfFirstBlendedImageSet(pageId, postId) {
  ...
  if (PageStreamMap.length) {
    PageStreamMap = Array.isArray(PageStreamMap[0].SelectedBlendImages)
      ? PageStreamMap[0].SelectedBlendImages
      : [];
...
async function __addCommentsToStreamPost_StreamTool(
  SocialPageId,
  SocialPostId,
  postCommentsArr
) {
  console.log(
    "###########################################------ __addCommentsToStreamPost_StreamTool --------#######################################"
  );
  const hexcode_blendedImage = await __getPostHexCodeOfFirstBlendedImageSet(
    SocialPageId,
    SocialPostId
  );
  console.log(
    "__getPostHexCodeOfFirstBlendedImageSet -------------------------------- ",
    hexcode_blendedImage
  );
  for (let i = 0; i < postCommentsArr.length; i++) {
    ...
    if (
      ...
      var dataToSave = {
        ...
        SocialPostId: SocialPostId ? new ObjectId(SocialPostId) : null,
        hexcode_blendedImage: hexcode_blendedImage
          ? hexcode_blendedImage
          : null,
      ...
      if (CommentId) {
        ...
        postCommentsArr[i].likesByEmailIds =
          postCommentsArr[i].likesByEmailIds || [];
        postCommentsArr[i].likes = postCommentsArr[i].likes || 0;
        if (
          postCommentsArr[i].likesByEmailIds.length &&
          postCommentsArr[i].likes
        ) {
          await __addLikesToStreamComments_StreamTool(
            SocialPageId,
            CommentId,
            postCommentsArr[i].likesByEmailIds,
            postCommentsArr[i].likes
...
async function __addCommentsToStreamPost_StreamTool2(
  SocialPageId,
  SocialPostId,
  blendImage1,
  blendImage2,
  ...
  try {
    ...
    if (pageId && mediaSelectionCriteria) {
      await Page.updateOne(
        { _id: new ObjectId(pageId) },
        { $set: { MediaSelectionCriteria: mediaSelectionCriteria } }
      );
      return res.json({
        code: 200,
        message: "MediaSelectionCriteria added successfully.",
      });
    }
  } catch (err) {
...
const addNewPost_INTERNAL_API = async (req, res) => {
  ...
  if (!ObjectIdConstructor) {
    ...
    return res.json({
  	...
  	try {
		const pageId = req.body.pageId || null;
		    let postStatement = req.body.postStatement || null;
		const MediaSelectionCriteria = req.body.MediaSelectionCriteria || null;
    ...
    try {
      const beforePostHeader = postStatement.substring(
        0,
        postStatement.lastIndexOf("[")
      );
      const postHeader = postStatement.substring(
        postStatement.lastIndexOf("[") + 1,
        postStatement.lastIndexOf("]")
      );
      const afterPostHeader = postStatement.substring(
        postStatement.lastIndexOf("]") + 1
      );
      
      if (postHeader) {
        postStatement = beforePostHeader + `<span style="font-family: Gambarino-Regular;">${postHeader}</span>` + afterPostHeader;
      } else {
    ...
    const postStreamObj = req.body.postStreamObj || {};
    const postStreamType = postStreamObj.type || "";
    const MJImageArr = postStreamObj.FileArr || postStreamObj.fileArr || [];
    
    // Debug: Log the structure of MJImageArr
    // console.log("=== MJImageArr DEBUG ===");
    // console.log("MJImageArr length:", MJImageArr.length);
    // if (MJImageArr.length > 0) {
    //   console.log("MJImageArr[0] structure:", Object.keys(MJImageArr[0]));
    //   console.log("MJImageArr[0].imageData exists:", !!MJImageArr[0].imageData);
    //   console.log("MJImageArr[0].imageData length:", MJImageArr[0].imageData ? MJImageArr[0].imageData.length : "N/A");
    //   console.log("MJImageArr[0].fileName:", MJImageArr[0].fileName);
    // }
    if (MJImageArr.length > 1) {
      // console.log("MJImageArr[1] structure:", Object.keys(MJImageArr[1]));
      // console.log("MJImageArr[1].imageData exists:", !!MJImageArr[1].imageData);
      // console.log("MJImageArr[1].imageData length:", MJImageArr[1].imageData ? MJImageArr[1].imageData.length : "N/A");
      // console.log("MJImageArr[1].fileName:", MJImageArr[1].fileName);
    }
    // console.log("=== END MJImageArr DEBUG ===");

    const postKeywords = req.body.keywords || "";
    
    // Log incoming keywords from API
    const allKeywordsArr = postKeywords.split(":::").map((obj) => obj.trim());
    let pArr = [];
    let sArr = [];
    let s2Arr = [];
    ...
    let objectId;
    try {
      objectId = new ObjectIdConstructor(pageId);
    } catch (objectIdError) {
      return res.json({
        code: 400,
    ...
    if (postStreamType === "1MJPost" || postStreamType === "2MJPost") {
      ...
      type = postStreamType || "Notes";
      name = `${postStreamType?.toLowerCase() || "post"}_${incNum}`;
    }

    let locationArray = [];
    
    // Helper function to generate S3 URL for MJ images
    const generateS3Url = (fileName, size = "aspectfit") => {
      const bucket = process.env.AWS_BUCKET_NAME || "scrpt";
      const region = process.env.AWS_REGION || "us-east-1";
      return `https://${bucket}.s3.${region}.amazonaws.com/scrptMedia/img/${size}/${fileName}`;
    };
    
    // Helper function to generate multiple S3 URLs for different image sizes
    const generateMultipleS3Urls = (fileName) => {
      const sizes = ["100", "300", "600", "aspectfit_small", "aspectfit"];
      const urls = {};
      sizes.forEach((size) => {
        urls[size] = generateS3Url(fileName, size);
      });
      return urls;
    };
    ...
    if (postStreamType === "1MJPost" && MJImageArr && MJImageArr.length === 1) {
      ...
      // console.log("=== UPLOADING MJ IMAGES TO S3 ===");
      // console.log("MJImageArr length:", MJImageArr.length);

      if (MJImageArr.length < 2) {
        // console.log(
        //   "⚠️ WARNING: 2MJPost requires 2 images, but only",
        //   MJImageArr.length,
        //   "provided"
        // );
      }
      
      for (let i = 0; i < MJImageArr.length; i++) {
        const imageData = MJImageArr[i];
        // console.log(`Processing MJ Image ${i + 1}:`, {
        //   fileName: imageData.fileName,
        //   hasBase64: !!imageData.base64,
        //   hasBuffer: !!imageData.buffer,
        //   hasData: !!imageData.data,
        //   hasImageData: !!imageData.imageData,
        //   imageDataPreview: imageData.imageData
        //     ? imageData.imageData.substring(0, 5) + "..."
        //     : "none",
        //   allFields: Object.keys(imageData) // Log all available fields
        // });
        
        // Check if we have actual image data to upload
        if (imageData.base64 || imageData.buffer || imageData.data || imageData.imageData) {
          try {
            // Convert base64 to buffer if needed
            let imageBuffer;
            if (imageData.base64) {
            ...
            console.log(
              `❌ MJ Image ${i + 1} upload failed, using fallback URL:`,
              imageUrl
            );
          }
        } else if (imageData.fileId) {
          ...
          try {
            // console.log(`🔄 Fetching MJ Image ${i + 1} from Google Drive...`);
            const imageBuffer = await awsS3Utils.fetchImageFromGoogleDrive(
              imageData.fileId
            );
            ...
            console.log(
              `❌ MJ Image ${i + 1} Google Drive fetch failed, using fallback URL:`,
              imageUrl
            );
            console.log(`Google Drive error:`, gdError.message);
          }
        } else {
          const imageUrl = generateS3Url(imageData.fileName);
          locationArray.push({ Size: "", URL: imageUrl });
          ...
          console.log(
            `⚠️ MJ Image ${i + 1} no data found, using generated URL:`,
            imageUrl
          );
        }
      }

      // console.log("=== MJ IMAGES UPLOAD COMPLETE ===");
    ...
    const dataToUpload = {
      ...
      GroupTags: groupTagsArray, // ✅ Now using processed GroupTags from Excel
      Collection: [
    ...
    if (
      pArr.length > 1 &&
      typeof pArr[0] === "string" &&
      typeof pArr[1] === "string"
    ) {
      console.log("✅ Using provided keywords (no AI extraction needed)");
      keywordsObj.Primary = [pArr[0], pArr[1]];
    ...
    const keywords = keywordsObj.Primary || [];
    ...
    // Process all keywords (primary + secondary + secondary2) for ranking
    // Now all keyword variables are available
    var allKeywordsForRanking = [
      ...(keywords || []),
      ...(secondaryKeywords || []),
      ...(secondaryKeywords2 || [])
    ];
    ...
    req.body.selectedKeywords = req.body.selectedKeywords || [];
    req.body.generatedKeywords = req.body.generatedKeywords || [];
    ...
    if (hasRankingData) {
      ...
      // groupTags already imported at top of file
      
      var selectedWords = [];
      for (var i = 0; i < allKeywordsForRanking.length; i++) {
        selectedWords.push({
          GroupTagTitle: { $regex: new RegExp("^" + allKeywordsForRanking[i].trim() + "$", "i") }
        });
      }
      ...
      // Process all keywords and assign individual ranks (limit to 5 ranks for performance)
      var maxRanks = 5; // Limit ranks to prevent complex MongoDB aggregation
      for (var i = 0; i < allKeywordsForRanking.length && rankCounter <= maxRanks; i++) {
        var keyword = allKeywordsForRanking[i].toLowerCase().trim();
        
        // Skip empty or undefined keywords
      ...
      // If we have remaining keywords, group them in the last rank
      if (rankCounter <= maxRanks) {
        for (var i = rankCounter * 4; i < allKeywordsForRanking.length; i++) {
          var keyword = allKeywordsForRanking[i].toLowerCase().trim();
          if (!keyword || keyword === 'undefined') {
            continue;
          }
    ...
    // Add primary keywords to rank 999 (highest priority) (like old code: req.body.selectedWords)
    subsetByRankObj2["999"] = keywords && keywords.length > 0 ? keywords[0] : "";
    ...
    req.body.subsetByRankObj2 = req.body.subsetByRankObj2 || {};
          ...
          // CRITICAL FIX: Both images must use the SAME rank data (like old code)
          // The old code creates rank data ONCE and both images use the same data
    ...
    console.log("🎯 req.body.subsetByRankObj2:", req.body.subsetByRankObj2);
      ...
      const reqObj = {
        PageId: pageId ? pageId : null,
        PostId: mediaId ? mediaId : null, // Use mediaId instead of thisPostId
        SurpriseSelectedWords: keywords ? keywords : [],
      ...
      if (mediaId && postStreamType === "1MJPost" && MJImageArr.length === 1) {
        const fileId = MJImageArr[0].fileId || "";
        const fileName = MJImageArr[0].fileName || "";
        const prompt = MJImageArr[0].prompt || "";
        const lightness = MJImageArr[0].lightness || 0;
        const title = MJImageArr[0].title || "";
        const photographer = MJImageArr[0].photographer || "";
        const source = MJImageArr[0].source || "";
        
        try {
          // Create a mock request object for the function call
          const mockReq = {
            query: {
              postId: mediaId, // Use mediaId instead of thisPostId
              fileId: fileId,
              fileName: fileName,
              prompt: prompt,
              lightness: lightness,
          ...
          await mediaController.syncGdMjImage_INTERNAL_API(mockReq, mockRes);
        } catch (error) {
          responseData = { code: null };
        }
        ...
        const fileId1 = MJImageArr[0].fileId || "";
        const fileName1 = MJImageArr[0].fileName || "";
        const prompt1 = MJImageArr[0].prompt || "";
        const lightness1 = MJImageArr[0].lightness || 0;
        const title1 = MJImageArr[0].title || "";
        const photographer1 = MJImageArr[0].photographer || "";
        const source1 = MJImageArr[0].source || "";

        const fileId2 = MJImageArr[1].fileId || "";
        const fileName2 = MJImageArr[1].fileName || "";
        const prompt2 = MJImageArr[1].prompt || "";
        const lightness2 = MJImageArr[1].lightness || 0;
        const title2 = MJImageArr[1].title || "";
        const photographer2 = MJImageArr[1].photographer || "";
        const source2 = MJImageArr[1].source || "";

        if (fileId1 && fileId2 && fileName1 && fileName2) {
          const reqObj = {
            PostId: mediaId || null, // Use mediaId instead of thisPostId
            inputArr: [
              {
                fileId: fileId1,
                fileName: fileName1,
          ...
          try {
            ...
            // Call the function directly
            await mediaController.syncGdTwoMjImage_INTERNAL_API(
              mockReq,
              mockRes
            );
          } catch (error) {
      ...
      if (postCommentsArr.length) {
        ...
        setTimeout(async () => {
          await __addCommentsToStreamPost_StreamTool(
            pageId,
            mediaId, // Use mediaId instead of thisPostId
            postCommentsArr
          );
        }, 10000);
      }
    } else {
      console.log("⚠️ Primary keywords length is not 2 - skipping blend images API call");
      console.log("⚠️ Keywords length:", keywords.length);
    ...
    console.log("❌ === addNewPost_INTERNAL_API ERROR ===");
    console.log("❌ Error Message:", err.message);
    console.log("❌ Error Stack:", err.stack);
    console.log("❌ Error Name:", err.name);
    console.log("❌ Request Body at Error: [Truncated for readability]");
...
var addGTAsyncAwait__INTERNAL_API = async function (req, res) {
  ...
  //console.log("Request body logging commented out");
  ...
  try {
    ...
    //console.log("Keywords array logging commented out");
    
    if (keywords.length === 0) {
      console.log("⚠️ WARNING: No keywords provided in request body");
      return res.json({
        code: 400,
        message: "No keywords provided",
        newkeywordsProcessed: [],
        existingkeywordsProcessed: [],
      });
    }

    for (var i = 0; i < keywords.length; i++) {
      var keywordObj = keywords[i] || {};
      //console.log("Keyword object logging commented out");
      
      if (keywordObj) {
        var keyword =
          typeof keywordObj.keyword === "string"
            ? keywordObj.keyword.trim().toLowerCase()
            : "";
        var tags =
          typeof keywordObj.tags === "string" ? keywordObj.tags.split(",") : [];
        
        var finalTags = tags.filter(
          (tag) => tag.trim().toLowerCase() !== keyword
        );
        console.log("✅ Final Tags (filtered):", finalTags);

        // Database query logging
        console.log("🔍 Searching for existing group tag with title:", keyword);
        var data = await groupTags
          .find({
        ...
        console.log("📊 Database Query Result Count:", data.length);
        if (data.length > 0) {
          console.log("📊 Existing Group Tag Found:", {
            _id: data[0]._id,
            GroupTagTitle: data[0].GroupTagTitle,
            status: data[0].status,
            TagsCount: data[0].Tags ? data[0].Tags.length : 0,
          });
        }

        if (data.length == 0) {
          console.log("🆕 Creating NEW group tag for keyword:", keyword);
          ...
          // Process tags with logging
          var processedTags = [{ TagTitle: keyword, status: 1 }];
          console.log("🏷️ Base Tag Added:", { TagTitle: keyword, status: 1 });
          
          var additionalTags = finalTags
            .map((tag) => {
            tag = typeof tag === "string" ? tag.trim().toLowerCase() : "";
            const tagType = getTagTypeByTagName(tag);
              console.log("🏷️ Processing Additional Tag:", {
                original: tag,
                tagType,
              });
            ...
            tag = tag
              .replace("Synonyms___", "")
              .replace("Metaphors___", "")
              .replace("Objects___", "")
              .replace("Feelings___", "")
              .replace("Adjectives___", "");
          ...
          doc.Tags = processedTags.concat(additionalTags);
          doc.Think = [];
          doc.Less = [];
          doc.More = [];
          doc.IsExistsOnExcel = true;
          
          //console.log("Document logging commented out");
          
          try {
            const savedDoc = await groupTags(doc).save();
            console.log("✅ NEW Group Tag Saved Successfully:", {
              _id: savedDoc._id,
              GroupTagTitle: savedDoc.GroupTagTitle,
              TagsCount: savedDoc.Tags.length,
            });
            newkeywordsProcessed.push(keyword);
          } catch (saveError) {
            console.error("❌ ERROR Saving NEW Group Tag:", saveError);
            throw saveError;
          }
        } else {
          console.log("🔄 Updating EXISTING group tag for keyword:", keyword);
          
          //update existing keyword record
          var alreadySavedKeyword = data[0];
          var uniqueTags = {};
          alreadySavedKeyword.Tags = Array.isArray(alreadySavedKeyword.Tags)
            ? alreadySavedKeyword.Tags
            : [];
          
          console.log(
            "🏷️ Existing Tags Count:",
            alreadySavedKeyword.Tags.length
          );
          
          alreadySavedKeyword.Tags.forEach((tag) => {
            tag.TagTitle =
              typeof tag.TagTitle === "string"
                ? tag.TagTitle.trim().toLowerCase()
                : "";
            tag.TagTitle = tag.TagTitle.replace("synonyms___", "")
              .replace("metaphors___", "")
              .replace("objects___", "")
              .replace("feelings___", "")
              .replace("adjectives___", "")
              .replace("feelings__", "")
              .replace("adjectives__", "");
            if (tag.TagTitle) {
              uniqueTags[tag.TagTitle] = tag;
            }
          });
          
          console.log("🏷️ Unique Existing Tags:", Object.keys(uniqueTags));

          finalTags.forEach((tag) => {
            tag = typeof tag === "string" ? tag.trim().toLowerCase() : "";
            const tagType = getTagTypeByTagName(tag);
            console.log("🏷️ Processing New Tag for Update:", {
              original: tag,
              tagType,
            });
            ...
            if (tag) {
              ...
              console.log("✅ New Tag Added to Unique Tags:", {
                TagTitle: tag,
                status: 1,
                TagType: tagType,
              });
          ...
          console.log("🏷️ Final Tags for Update:", tagDocs.length);
          
          try {
            const updateResult = await groupTags.updateOne(
              { _id: new ObjectId(alreadySavedKeyword._id) },
              { $set: { IsExistsOnExcel: true, Tags: tagDocs } }
            );
            console.log("✅ EXISTING Group Tag Updated Successfully:", {
              _id: alreadySavedKeyword._id,
              MatchedCount: updateResult.matchedCount,
              ModifiedCount: updateResult.modifiedCount,
            });
            existingkeywordsProcessed.push(keyword);
          } catch (updateError) {
            console.error("❌ ERROR Updating EXISTING Group Tag:", updateError);
            throw updateError;
          }
        }
      } else {
        console.log("⚠️ WARNING: Empty keyword object at index:", i);
      }
    }
    ...
    console.log(
      "🆕 New Keywords Processed:",
      newkeywordsProcessed.length,
      newkeywordsProcessed
    );
    console.log(
      "🔄 Existing Keywords Updated:",
      existingkeywordsProcessed.length,
      existingkeywordsProcessed
    );
    console.log("✅ addGTAsyncAwait__INTERNAL_API COMPLETED SUCCESSFULLY");

    return res.json({
      code: 200,
      newkeywordsProcessed: newkeywordsProcessed,
      existingkeywordsProcessed: existingkeywordsProcessed,
      executionTime: executionTime + "ms",
      totalKeywordsProcessed: keywords.length,
    });
  } catch (error) {
    const executionTime = Date.now() - startTime;
    console.error("❌ CRITICAL ERROR in addGTAsyncAwait__INTERNAL_API:");
...
let downloadStreamPostMetaData_INTERNAL_API = async function (req, res) {
  ...
  const perPostFreq =
    PageStreamMap.length > 365 ? 1 : parseInt(365 / PageStreamMap.length) + 1;
  ...
  for (let i = 0; i < PageStreamMap.length; i++) {
    ...
    let perPostLimit =
      postObj.SelectedBlendImages.length > perPostFreq
        ? perPostFreq
        : postObj.SelectedBlendImages.length;
    for (let j = 0; j < perPostLimit; j++) {
      let selectedPostBlendObject = postObj.SelectedBlendImages[j] || {};
      selectedPostBlendObject.MetaData_1 =
        selectedPostBlendObject.MetaData_1 || {};
      selectedPostBlendObject.MetaData_1.BriefDescription =
        selectedPostBlendObject.MetaData_1.BriefDescription || "";
      selectedPostBlendObject.MetaData_1.DetailedDescription =
        selectedPostBlendObject.MetaData_1.DetailedDescription || "";
      selectedPostBlendObject.MetaData_1.AestheticDescription =
        selectedPostBlendObject.MetaData_1.AestheticDescription || "";
      selectedPostBlendObject.MetaData_1.GoogleDriveFilename =
        selectedPostBlendObject.MetaData_1.GoogleDriveFilename || "";

      selectedPostBlendObject.MetaData_2 =
        selectedPostBlendObject.MetaData_2 || {};
      selectedPostBlendObject.MetaData_2.BriefDescription =
        selectedPostBlendObject.MetaData_2.BriefDescription || "";
      selectedPostBlendObject.MetaData_2.DetailedDescription =
        selectedPostBlendObject.MetaData_2.DetailedDescription || "";
      selectedPostBlendObject.MetaData_2.AestheticDescription =
        selectedPostBlendObject.MetaData_2.AestheticDescription || "";
      selectedPostBlendObject.MetaData_2.GoogleDriveFilename =
        selectedPostBlendObject.MetaData_2.GoogleDriveFilename || "";
      ...
      jsonArr.push({
        ...
        UploadedFileName_2:
          selectedPostBlendObject.MetaData_2.GoogleDriveFilename,
        BriefDescription_1: selectedPostBlendObject.MetaData_1.BriefDescription,
        DetailedDescription_1:
          selectedPostBlendObject.MetaData_1.DetailedDescription,
        AestheticDescription_1:
          selectedPostBlendObject.MetaData_1.AestheticDescription,
        BriefDescription_2: selectedPostBlendObject.MetaData_2.BriefDescription,
        DetailedDescription_2:
          selectedPostBlendObject.MetaData_2.DetailedDescription,
        AestheticDescription_2:
          selectedPostBlendObject.MetaData_2.AestheticDescription,
      });
...
var addComments_INTERNAL_API = async function (req, res) {
  ...
  if (PostCommentsArr.length) {
    ...
    return res.json({
      code: 200,
      message: "Comments and Likes added successfully.",
    });
  }
...
const deleteAllPosts_BROWSER_API = async function (req, res) {
  try {
    ...
    let result = await Page.updateOne(conditions, setObj);
    result.ok = result.ok || 0;
    if (result.ok) {
      //delete the post stream as well
      await PageStream.remove({ PageId: new ObjectId(pageId) });
...
const deleteAllComments_BROWSER_API = async function (req, res) {
  try {
    var pageId = req.query.PageId || null;
    var conditions = {
      SocialPageId: new ObjectId(pageId),
    };
    ...
    return res.json({
      code: "200",
      message: "All Comments and Likes deleted successfully!",
    });
  } catch (err) {
    console.log(err);
    return res.json({ code: "501", message: "Something went wrong." });
...
const updateStreamPostWithTransparentImage = async function (req, res) {
  /*let PageId = req.body.pageId || null;
	...
	//create new PageStream config replica
	var PageStreamDataArr = await PageStream.find({PageId : new ObjectId(PageId), PostId : new ObjectId(PostId)}).lean();
...
exports.getPrimarySecondaryKeywordsPrompt_BROWSER_API =
  getPrimarySecondaryKeywordsPrompt_BROWSER_API;
exports.updatePrimarySecondaryKeywordsPrompt_BROWSER_API =
  updatePrimarySecondaryKeywordsPrompt_BROWSER_API;
...
exports.updateStreamMediaFilterSortingOrder_BROWSER_API =
...
exports.updatePostActionAnnouncementSeenLog =
