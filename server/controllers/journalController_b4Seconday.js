				...
				EmailTemplate.find(condition, {}, function (err, results) {
					if (!err) {
						if (results.length) {
							...
							var PostURL = "https://scrpt.com/post_view?post="+dataRecord.PostId;
	...
	Media.aggregate([
		...
		if (results.length == 1) {
			...
			SurpriseImagesFrequencyWise.push(
				{
					BlendImage1 : BlendImage1.MediaURL ? BlendImage1.MediaURL : "https://scrpt.com/assets/Media/img/600/"+BlendImage1.MediaURL2[0].URL,
					BlendImage2 : BlendImage2.MediaURL ? BlendImage2.MediaURL : "https://scrpt.com/assets/Media/img/600/"+BlendImage2.MediaURL2[0].URL
				}
			);
		...
		if (results.length == 1) {
			...
			for(var i = 0; i < EmailEngineDataSets.length; i++) {
				...
				if(!EmailEngineDataSets[i].VisualUrls.length) {
					...
					EmailEngineDataSets[i].VisualUrls.push((BlendImage1.MediaURL ? BlendImage1.MediaURL : "https://scrpt.com/assets/Media/img/600/"+BlendImage1.MediaURL2[0].URL));
					EmailEngineDataSets[i].VisualUrls.push((BlendImage2.MediaURL ? BlendImage2.MediaURL : "https://scrpt.com/assets/Media/img/600/"+BlendImage2.MediaURL2[0].URL));
				}
			}
...
function generateBlendImage (blendImage1, blendImage2, blendOption, uploadDir, file_name) {
	...
	if(blendImage1 && blendImage2 && blendOption){
		if(blendImage1.indexOf('unsplash.com') == -1){
			...
			blendImage1 = blendImage1.replace('https://scrpt.com', '');
			blendImage1 = blendImage1.replace('http://scrpt.com', '');
		...
		function nextStep_1() {
			if(blendImage2.indexOf('unsplash.com') == -1){
				...
				blendImage2 = blendImage2.replace('https://scrpt.com', '');
				blendImage2 = blendImage2.replace('http://scrpt.com', '');
...
async function sendCoffeeInvitationEmail (memberObj, OwnerDetails, PostStatement, VisualUrls, BlendMode) {
...
var stream__sendCoffeeInvitation = async function (req, res) {
	...
	for(var i = 0; i < members.length; i++) {
		...
		sendCoffeeInvitationEmail(member, OwnerDetails, PostStatement, VisualUrls, BlendMode);
	}
...
var addCompiledStatementsToGoogleSheet = async function (req, res) {
	...
	if(PageData) {
		...
		for(var i = 0; i < PageData.Medias.length; i++) {
			...
			//console.log("PostStatement = ", PostStatement);
			if(PostStatement) {
		...
		if(PostArr.length) {
			...
			console.log("Calling http://www.scrpt.com:3000/task/addRows --------------------------------- ", reqObj);
			var request_url = 'http://www.scrpt.com:3000/task/addRows';
			axios.post(request_url, reqObj)
			.then(response => {
				response.data = response.data ? response.data : {};
				response.data.code = response.data.code ? response.data.code : null;
				console.log("------------ AXIOS (http://www.scrpt.com:3000/task/addRows)---- http://www.scrpt.com:3000/task/addRows api response ---------------", response.data.code);
			}).then(error => {
				console.log("error = ", error);
			});
			*/
