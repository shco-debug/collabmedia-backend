...
async function mapPostsAsPerSettings(CapsuleData, firstTimeFlag) {
	...
	try {
		...
		var __StreamTYPE = CapsuleData.LaunchSettings.StreamType ? CapsuleData.LaunchSettings.StreamType : '';
		...
		if(firstTimeFlag) {
			...
			var allSyncedPosts = await SyncedPost.find(conditions).sort({"EmailEngineDataSets.DateOfDelivery" : 1});
			...
			for(var i = 0; i < EmailEngineDataSets.length; i++) {
				...
				EmailEngineDataSets[i].DateOfDelivery = __StreamTYPE == 'Group' ? getDateIncrementedBy_CreatedOn_GroupStream(NoOfDays, CreatedOn) : getDateIncrementedBy_CreatedOn(NoOfDays, CreatedOn, 5);
				...
				if(allSyncedPostsMap.SyncedPosts[i]) {
					...
					tmpObj.EmailEngineDataSets[0].AfterDays = EmailEngineDataSets[i].AfterDays;
					tmpObj.EmailEngineDataSets[0].DateOfDelivery = EmailEngineDataSets[i].DateOfDelivery;
			...
			var CreatedOn = (__StreamTYPE === 'Group' && CapsuleData.LaunchDate) ? new Date(String(CapsuleData.LaunchDate)) : new Date(String(CapsuleData.CreatedOn));
			var EmailEngineDataSets = __getEmailEngineDataSetsBasedOnMonthAndFreq (CapsuleData);
			...
			for(var i = 0; i < EmailEngineDataSets.length; i++) {
				...
				if(EmailEngineDataSets[i].DateOfDelivery_Timestamp >= datePlusOneTimestamp && allSyncedPostsMap[i]) {
					allSyncedPostsMap.SyncedPosts = allSyncedPostsMap.SyncedPosts || [];
					if(allSyncedPostsMap.SyncedPosts[i]) {
						var tmpObj = allSyncedPostsMap.SyncedPosts[i];
						tmpObj.EmailEngineDataSets[0].AfterDays = EmailEngineDataSets[i].AfterDays;
...
async function AreSyncedPostsAlreadyThere (StreamId) {
	...
	try {
		var alreadySavedRecords = await SyncedPost.find({CapsuleId: ObjectId(StreamId)});
		alreadySavedRecords = Array.isArray(alreadySavedRecords) ? alreadySavedRecords : [];
...
async function __streamPagePostNow(PagePosts, PageData, shareWithEmail, req, CapsuleData) {
	console.log("----------------- __streamPagePostNow STARTED ----------------------------");
	console.log(`--- __streamPagePostNow --- ${shareWithEmail} ---- ${CapsuleData._id} ---- ${PagePosts.length}`)
	...
	var EmailEngineDataSets = __getEmailEngineDataSetsBasedOnMonthAndFreq_AllPosts (CapsuleData);
	...
	for(let loop365 = 0; loop365 < PagePosts2.length; loop365++) {
		...
		for( let i365 = 0; i365 < inputObj.EmailEngineDataSets.length; i365++ ) {
			inputObj.EmailEngineDataSets[i365].TextAboveVisual = inputObj.EmailEngineDataSets[i365].TextAboveVisual ? inputObj.EmailEngineDataSets[i365].TextAboveVisual.replace(/\n/g,'<br />') : '';
			inputObj.EmailEngineDataSets[i365].TextBelowVisual = inputObj.EmailEngineDataSets[i365].TextBelowVisual ? inputObj.EmailEngineDataSets[i365].TextBelowVisual.replace(/\n/g,'<br />') : '';
		}
	...
	var EmailEngineDataSets_keyposts = __getEmailEngineDataSetsBasedOnMonthAndKeyPost_AllPosts (CapsuleData);
	...
	for(let loop366 = 0; loop366 < PagePosts_keyposts.length; loop366++) {
		...
		for( let i366 = 0; i366 < inputObj.EmailEngineDataSets.length; i366++ ) {
			inputObj.EmailEngineDataSets[i366].TextAboveVisual = inputObj.EmailEngineDataSets[i366].TextAboveVisual ? inputObj.EmailEngineDataSets[i366].TextAboveVisual.replace(/\n/g,'<br />') : '';
			inputObj.EmailEngineDataSets[i366].TextBelowVisual = inputObj.EmailEngineDataSets[i366].TextBelowVisual ? inputObj.EmailEngineDataSets[i366].TextBelowVisual.replace(/\n/g,'<br />') : '';
		}
	...
	console.log("----------------- __streamPagePostNow COMPLETED now actually making the map as per settings ----------------------------");

	if(await mapPostsAsPerSettings(CapsuleData, firstTimeFlag)) {
		console.log("----------------- __streamPagePostNow SUCCESSFULLY COMPLETED for NORMAL Stream Case - BUY_NOW Case ----------------------------");
	} else {
...
var capsule__createNewInstance = function (CapsuleData, owner, req, index_value_email) {
						...
						Chapter.find(conditions, fields).sort(sortObj).exec(function (err, results) {
							if (!err) {
														...
														for (var loop = 0; loop < results.length; loop++) {
															...
															Page.findOne(conditions, fields, async function (err, result) {
																...
																if (data.PageType == "content") {
																	...
																	//console.log("data = ", data);
																	Page(data).save(async function (err, result) {
																		if (!err) {
																			...
																			if(CapsuleData.LaunchSettings.CapsuleFor == 'Stream' && CapsuleData.LaunchSettings.StreamType != 'Group') {
																				if(result.Medias.length) {
																					...
																					console.log(`calling __streamPagePostNow for ${shareWithEmail} capsule id = ${CapsuleData._id} total posts = ${data.Medias.length}`);
																					await __streamPagePostNow(result.Medias, result, shareWithEmail, req, CapsuleData);
																				} else {
								...
								*/
								...
								EmailTemplate.find(condition, {}, async function (err, results) {
									if (!err) {
										if (results.length) {
											setTimeout(async function(){
												var RecipientName = shareWithName ? shareWithName : '';
												User.find({ 'Email': new RegExp('^'+shareWithEmail+'$', 'i'), IsDeleted: false }, { 'Name': true, 'AllFoldersId' : true, 'AllPagesId': true }, async function (err, userD) {
													var _cId = '';
													var _pId = '';
				...
				if (order.CartItems.length > 0) {
					...
					if (userCreditAmount.CreditAmount != 0 && userCreditAmount.IsCredit == true) {
						...
						if (userCreditAmount.CreditAmount <= order.TotalPayment) {
							...
							//console.log("111122222222222222")
...
function capsule__createNewInstances_perCartItem(__capsuleId, cartItemOwners, cartItem, req, callback) {
	var conditions = {
		_id: __capsuleId
	}
	var fields = {};
		...
		if (!err) {
			if (results.length) {
				...
				if (cartItemOwners.length) {
					for (var ctloop = 0; ctloop < cartItemOwners.length; ctloop++) {
						var owner = {
							UserName: cartItemOwners[ctloop].OwnerName ? cartItemOwners[ctloop].OwnerName : null,
							UserEmail: cartItemOwners[ctloop].OwnerEmail ? cartItemOwners[ctloop].OwnerEmail : null,
							UniqueIdPerOwner: cartItemOwners[ctloop].UniqueIdPerOwner ? cartItemOwners[ctloop].UniqueIdPerOwner : null
						};

						//var UniqueIdPerOwner = UniqueIds[ctloop] ? UniqueIds[ctloop] : null;
						capsule__createNewInstance(CapsuleData, owner, req, ctloop);
					}
				}
				else {
					var response = {
...
var savedOrder = function (charge, orderInit, payFrom, rdm_credit, result, order, req, res) {
	if (payFrom == 'stripe') {
		orderInit.PGatewayResult = charge;
	}
	//console.log("gfdgdgfdgfdgdf", charge, orderInit, payFrom, rdm_credit, result);

	//console.log("7777777777777", charge.paid && charge.failure_code == null);

	if (charge.paid && charge.failure_code == null || payFrom == 'free') {
		//console.log("gfdgdgfdgfdgdf-------7878787878787878------");
		orderInit.TransactionState = 'Completed';
		var message = "Payment completed successfully";
		var status = 200;
		...
		for (var loop = 0; loop < order.CartItems.length; loop++) {
			...
			capsule__createNewInstances_perCartItem(__capsuleId, cartItemOwners, cartItem, req, function (err, result) {
				totalOps++;
				//if( !err ){
